...
local PCMainMiniMap = class("PCMainMiniMap", BaseFGUILayout)
...
function PCMainMiniMap:Create()
	self._ui = FGUI:ui_delegate(FGUI:GetChild(self.component, "minimap"))
    ...
    self._tracePoint:Create("Main_pc", "MiniMapTracePointItem", handler(self, self.CalcMiniMapPos), 15)
    ...
    FGUI:setOnClickEvent(self._ui.Btn_increase, handler(self, self.OnIncreaseMap))
...
function PCMainMiniMap:Enter()
    ...
    FGUI:GButton_setSelected(self._ui.Btn_music, SL:GetValue("SETTING_MUTE_BGM"))
end

function PCMainMiniMap:Exit()
...
function PCMainMiniMap:Destroy()
    self._tracePoint:Destroy()
    self._ui = nil
    Pool.Destroy("Main_pc", "MiniMapPoint", false)
end


--------------------------------------------------------

function PCMainMiniMap:UpdatePointsTimer()
...
function PCMainMiniMap:UpdateMapData()
...
function PCMainMiniMap:UpdateMapName()
    FGUI:GTextField_setText(self._ui.Text_mapName, SL:GetValue("MAP_NAME") or "404")
end

function PCMainMiniMap:UpdateMapRoute()
    local lines = SL:GetValue("MAP_INFO_ROUTES", SL:GetValue("MAP_ID"))
    FGUI:setVisible(self._ui.Btn_route, lines and true or false)

    self:UpdateMapState()
end

function PCMainMiniMap:UpdatePlayerPos()
...
function PCMainMiniMap:UpdateMiniMap(x, y)
...
function PCMainMiniMap:UpdateMiniMapSize()
...
function PCMainMiniMap:UpdateMiniMapPos(x, y)
...
function PCMainMiniMap:InitActorPoints()
    if not self._mapAble then return end
...
function PCMainMiniMap:UpdatePlayerPoint(actorId)
...
function PCMainMiniMap:UpdateMonsterPoint(actorId)
...
function PCMainMiniMap:UpdateActorPoint(actorId)
...
function PCMainMiniMap:AddActorPoint(actorId, styleIdx, posID)
    ...
    if actorPoint then
        ...
        actorPoint = Pool.Get("Main_pc", "MiniMapPoint", self._mapUI.Node_actors)
        self._actorCounts[posID] = 1
        self._actorPosIDs[actorId] = posID
...
function PCMainMiniMap:RmvActorPoint(actorId)
...
function PCMainMiniMap:UpdateTeamMemberPoint(actorId)
...
function PCMainMiniMap:AddTeamMemberPoint(actorId)
    local actorPoint = self._teamPoints[actorId]
    if actorPoint then return end
    actorPoint = Pool.Get("Main_pc", "MiniMapPoint", self._mapUI.Node_team)
...
function PCMainMiniMap:RmvTeamMemberPoint(actorId)
    local actorPoint = self._teamPoints[actorId]
    if not actorPoint then return end
...
function PCMainMiniMap:UpdateActorPointState(actorPoint, styleIdx)
    local controller = FGUI:getController(actorPoint, "style")
    FGUI:Controller_setSelectedIndex(controller, styleIdx)
end

function PCMainMiniMap:UpdateActorPointPosition(actorId, actorPoint)
...
function PCMainMiniMap:GetActorPointPosID(actorId, z)
...
function PCMainMiniMap:UpdateMapPoints()
    ...
    for k, v in pairs(list) do
        if v and not SL:GetValue("NPC_IS_HIDE_POINT", v.ID) then
            ...
            local actorPoint = Pool.Get("Main_pc", "MiniMapPoint", self._mapUI.Node_npc)
    ...
    local createLinkPoint = function(posStr)
        ...
        if x and y then
            ...
            local actorPoint = Pool.Get("Main_pc", "MiniMapPoint", self._mapUI.Node_link)
            FGUI:setPosition(actorPoint, x, y)
...
function PCMainMiniMap:ClearPoints(points)
    local cells = points
    if #cells <= 0 then return end
    for k, cell in pairs(cells) do
        FGUI:setPosition(cell, -5000, -5000)
        Pool.Release("Main_pc", "MiniMapPoint", cell, false)
    end
    table.clear(points)
end

function PCMainMiniMap:CalcMainPlayerMiniMapPos(x, y)
...
function PCMainMiniMap:CalcMiniMapPos(actorX, actorZ)
...
----------------------------------------寻路-------------------------------------
function PCMainMiniMap:UpdateFindPath()
...
function PCMainMiniMap:FindPathBegin()
...
function PCMainMiniMap:ShowFindPathLine()
	...
	-- if size > 1 or dis > 1 then
        ...
        t[len + 2] = y
        len = len + 2
        FGUI:GComponent_DrawLine(self._mapUI.Node_line, "ui://Main_pc/point_green", 8, t)
	-- end
	end
end

function PCMainMiniMap:FindPathEnd()
	local size = SL:GetValue("MAP_PATH_SIZE")
	if size <= 0 then
...
function PCMainMiniMap:OnIncreaseMap()
    local ctrler = FGUI:getController(self._ui.nativeUI, "size")
    local idx = FGUI:Controller_getSelectedIndex(ctrler)
    local nextIdx = idx + 1
    if nextIdx > 2 then return end
    FGUI:Controller_setSelectedIndex(ctrler, nextIdx)
    self._limitSizeW, self._limitSizeH = FGUI:getSize(self._ui.Comp_Map)
    self:UpdateMiniMapSize()
end

function PCMainMiniMap:OnReduceMap()
    local ctrler = FGUI:getController(self._ui.nativeUI, "size")
    local idx = FGUI:Controller_getSelectedIndex(ctrler)
    local nextIdx = idx - 1
    if nextIdx < 0 then return end
    FGUI:Controller_setSelectedIndex(ctrler, nextIdx)
    if nextIdx > 0 then
        self._limitSizeW, self._limitSizeH = FGUI:getSize(self._ui.Comp_Map)
        self:UpdateMiniMapSize()
...
function PCMainMiniMap:OnClickMusic()
    local isMute = SL:GetValue("SETTING_MUTE_BGM")
    SL:SetValue("SETTING_MUTE_BGM", not isMute)
    FGUI:GButton_setSelected(self._ui.Btn_music, SL:GetValue("SETTING_MUTE_BGM"))
end

function PCMainMiniMap:OnClickAuto()
...
function PCMainMiniMap:OnOpenMapRoute()
    FGUI:Open("MapRoute_pc", "MapRoutePanel", nil, nil, {classPath = "FGUILayout/MapRoute/MapRoutePanel"})
end

function PCMainMiniMap:OnChangeMap(data)
...
function PCMainMiniMap:OnPlayerAction(actorID, act)
...
function PCMainMiniMap:OnActorInOfView(actorId)
    if SL:GetValue("ACTOR_IS_MONSTER", actorId) then
        self:UpdateMonsterPoint(actorId)
    elseif SL:GetValue("ACTOR_IS_PLAYER", actorId) then
        self:UpdatePlayerPoint(actorId)
...
function PCMainMiniMap:OnActorOutOfView(actorId)
    if SL:GetValue("ACTOR_IS_MONSTER", actorId) then
        self:RmvActorPoint(actorId)
    elseif SL:GetValue("ACTOR_IS_PLAYER", actorId) then
        if SL:GetValue("TEAM_IS_MEMBER", actorId) then
...
function PCMainMiniMap:OnMonsterDie(actorId)
    self:RmvActorPoint(actorId)
end
function PCMainMiniMap:OnMonsterRevive(actorId)
    self:UpdateMonsterPoint(actorId)
end

function PCMainMiniMap:OnTeamMemberUpdate()
...
function PCMainMiniMap:OnAFKBegin()
    self:UpdateAutoButton(true)
end
function PCMainMiniMap:OnAFKEnd()
    self:UpdateAutoButton(false)
end
function PCMainMiniMap:UpdateAutoButton(isAFK)
    if isAFK == nil then
...
-----------------------------------注册事件--------------------------------------
function PCMainMiniMap:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_MAP_STATE_CHANGE, "PCMainMiniMap", handler(self, self.UpdateMapState))
    SL:RegisterLUAEvent(LUA_EVENT_CHANGE_SCENE, "PCMainMiniMap", handler(self, self.OnChangeMap))
    SL:RegisterLUAEvent(LUA_EVENT_FIND_PATH_BEGAIN, "PCMainMiniMap", handler(self, self.FindPathBegin))
    SL:RegisterLUAEvent(LUA_EVENT_FIND_PATH_EMD, "PCMainMiniMap", handler(self, self.FindPathEnd))

    SL:RegisterLUAEvent(LUA_EVENT_PLAYER_DIRECTION_CHANGE, "PCMainMiniMap", handler(self, self.UpdatePlayerRotate))
    SL:RegisterLUAEvent(LUA_EVENT_PLAYER_MAPPOS_CHANGE, "PCMainMiniMap", handler(self, self.UpdatePlayerPos))
    SL:RegisterLUAEvent(LUA_EVENT_PLAYER_ACTION_PROCESS, "PCMainMiniMap", handler(self, self.OnPlayerAction))
    SL:RegisterLUAEvent(LUA_EVENT_PLAYER_ACTION_COMPLETE, "PCMainMiniMap", handler(self, self.OnPlayerAction))

    SL:RegisterLUAEvent(LUA_EVENT_ACTOR_IN_OF_VIEW, "PCMainMiniMap", handler(self, self.OnActorInOfView))
    SL:RegisterLUAEvent(LUA_EVENT_ACTOR_OUT_OF_VIEW, "PCMainMiniMap", handler(self, self.OnActorOutOfView))
    SL:RegisterLUAEvent(LUA_EVENT_MONSTER_DIE, "PCMainMiniMap", handler(self, self.OnMonsterDie))
    SL:RegisterLUAEvent(LUA_EVENT_MONSTER_REVIVE, "PCMainMiniMap", handler(self, self.OnMonsterRevive))
    
    SL:RegisterLUAEvent(LUA_EVENT_TEAM_MEMBER_UPDATE, "PCMainMiniMap", handler(self, self.OnTeamMemberUpdate))
    SL:RegisterLUAEvent(LUA_EVENT_AFK_BEGIN, "PCMainMiniMap", handler(self, self.OnAFKBegin))
    SL:RegisterLUAEvent(LUA_EVENT_AFK_END, "PCMainMiniMap", handler(self, self.OnAFKEnd))
end

function PCMainMiniMap:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_MAP_STATE_CHANGE, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_CHANGE_SCENE, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_FIND_PATH_BEGAIN, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_FIND_PATH_EMD, "PCMainMiniMap")

    SL:UnRegisterLUAEvent(LUA_EVENT_PLAYER_DIRECTION_CHANGE, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_PLAYER_MAPPOS_CHANGE, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_PLAYER_ACTION_PROCESS, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_PLAYER_ACTION_COMPLETE, "PCMainMiniMap")
    
    SL:UnRegisterLUAEvent(LUA_EVENT_ACTOR_IN_OF_VIEW, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_ACTOR_OUT_OF_VIEW, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_MONSTER_DIE, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_MONSTER_REVIVE, "PCMainMiniMap")

    SL:UnRegisterLUAEvent(LUA_EVENT_TEAM_MEMBER_UPDATE, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_AFK_BEGIN, "PCMainMiniMap")
    SL:UnRegisterLUAEvent(LUA_EVENT_AFK_END, "PCMainMiniMap")
end


return PCMainMiniMap