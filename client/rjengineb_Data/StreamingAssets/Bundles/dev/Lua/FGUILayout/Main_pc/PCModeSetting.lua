...
local PCModeSetting = class("PCModeSetting", BaseFGUILayout)

function PCModeSetting:Create()
...
function PCModeSetting:SelectMode()
...
function PCModeSetting:ClickMode1()
    self._findTargetMode = SLDefine.FIND_TARGET_MODE.PRIORITY_MONSTER
    self:SelectMode()
end

function PCModeSetting:ClickMode2()
    self._findTargetMode = SLDefine.FIND_TARGET_MODE.PRIORITY_PLAYER
    self:SelectMode()
end

function PCModeSetting:ClickMode3()
    self._findTargetMode = SLDefine.FIND_TARGET_MODE.ONLY_PLAYER
    self:SelectMode()
end

function PCModeSetting:Enter()
    self:InitView()
end

function PCModeSetting:Exit()

end

function PCModeSetting:Destroy()
    self._ui = nil	
end
...
function PCModeSetting:InitModeDatas()
...
function PCModeSetting:InitView()
...
function PCModeSetting:OnListModeRender(index, item)
    local idx = index + 1
