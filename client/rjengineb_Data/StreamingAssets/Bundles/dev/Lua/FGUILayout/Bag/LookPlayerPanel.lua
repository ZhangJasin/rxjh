...
local LookPlayerPanel = class("LookPlayerPanel", BaseFGUILayout)
...
function LookPlayerPanel:GetAllFGuiData()
...
function LookPlayerPanel:InitUI()
    ...
    self._pageList[4] = FGUI:CreateObject(self.list_property,"Bag","page4")
...
function LookPlayerPanel:RefreshPropertyUI()
...
function LookPlayerPanel:Destory()
    self:ClearPageList()
end

function LookPlayerPanel:ClearPageList()
...
function LookPlayerPanel:SetValueInText(component,attrName,attValue,showStr)
...
function LookPlayerPanel:Enter(page)
    self:RegisterEvent()
    if not page then
        page = IDX_PROPERTY
    end
...
function LookPlayerPanel:RefreshView()
...
function LookPlayerPanel:UpdatePlayerInfo()
    local data = SL:GetValue("L.M.PLAYER_DATA")
    if self._pageList[1] then
        local page1 = self._pageList[1]
        local mingZi = FGUI:GetChild(page1,"mingZi")
    ...
    if self._pageList[2] then
        ...
        if transformConfig and transformConfig.TransferName and transformConfig.TransferLV then
            ...
            for k,v in pairs(data.Abil) do
                ...
                if cfg then
                    ...
                    then
                        ...
                        if cfg.Type == 1 then
                            data.ShowValue = data.maxValue * 0.01 .. "%"
...
function LookPlayerPanel:AttrItemRender(idx, item)
    local attrData = self.attrDymicLoadTable[idx+1]
    self:SetValueInText(item,attrData.Name,attrData.ShowValue,attrData.Desc)
end

-- 进度条设置数值和进度
function LookPlayerPanel:SetProgressBar(component,barName,currentValue,maxValue,mode)
...
function LookPlayerPanel:Close()
	self.super.Close(self)
end
function LookPlayerPanel:InitData()
    self.equipMentObjList = {}
    self._scheduleSet = {}
end

function LookPlayerPanel:InitOnClickEvent()
...
function LookPlayerPanel:PageTo(index)
    ...
    self.ctrl_isShowTitle.selectedIndex = index == IDX_TITLE and 0 or 1
    self:RefreshView()
end
...
function LookPlayerPanel:TitleItemRender(idx,item)
    ...
    ctrl_isShowPin.selectedIndex = 1
...
function LookPlayerPanel:BtnTab1Clicked()
    self:PageTo(IDX_PROPERTY)
end

function LookPlayerPanel:BtnTab2Clicked()
    self:PageTo(IDX_TITLE)
end

function LookPlayerPanel:UpdatePlayerEquip()
    -- equips
    self:ClearAllEquipItem()
...
-- 释放所有挂载的Item
function LookPlayerPanel:ClearAllEquipItem()
...
function LookPlayerPanel:ClearModel()
    if self._model then
        self:UIModel_Unbind(self.model_root)
    end
end

function LookPlayerPanel:UpdateRoleModel()
    self:ClearModel()
...
function LookPlayerPanel:Destroy()
    self:ClearAllEquipItem()
end

-- 设置模型旋转
function  LookPlayerPanel:SetModelRotate(uiTouch)
    local angleX = 0
    local angleY = 0
    local angleZ = 0
    local beginX = nil
