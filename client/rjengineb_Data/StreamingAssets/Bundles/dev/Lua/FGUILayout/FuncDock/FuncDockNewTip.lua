...
local FuncDockNewTip = class("FuncDockNewTip", BaseFGUILayout)
local FuncDockUtil = requireFGUILayout("FuncDock/FuncDockUtil")
local MODEL_SCALE = 1
-- 角色面板弹窗
function FuncDockNewTip:Create()
...
function FuncDockNewTip:GetAllFGuiData()
...
function FuncDockNewTip:InitOnClickEvent()
    FGUI:setOnClickEvent(self.btn_close,handler(self,self.OnClose))
end

function FuncDockNewTip:InitGridLayout()
...
-- 可以根据类别动态设置按钮
function FuncDockNewTip:InitButtonData()
    -- merageData ---
...
function FuncDockNewTip:RefreshBtnGridLayout()
    FGUI:GList_setNumItems(self.grid_layout_btn, #self.btnTypes)
end

function FuncDockNewTip:Enter(data)
    if not data then
        SL:PrintEx("FuncDockNewTip data is nil")
        return
    end
...
function FuncDockNewTip:Exit()
    self:ClearModel()
    self:RemoveEvent()
end

function FuncDockNewTip:ClearModel()
    if self._model then
...
-- 刷新左边(人物模型)
function FuncDockNewTip:RefreshLeft()
    self:ClearModel()
...
-- 设置旋转
function FuncDockNewTip:SetModelRotate(uiTouch)
    local angleX = 0
    local angleX = 0
    local angleY = 0
    local angleZ = 0
...
-- 刷新右边
function FuncDockNewTip:RefreshRight()
...
function FuncDockNewTip:RefreshView()
...
-- 关闭面板
function FuncDockNewTip:OnClose()
    self.super.Close(self)
end
-----------------------------------注册事件--------------------------------------
function FuncDockNewTip:RegisterEvent()
end

function FuncDockNewTip:RemoveEvent()
end

return FuncDockNewTip
