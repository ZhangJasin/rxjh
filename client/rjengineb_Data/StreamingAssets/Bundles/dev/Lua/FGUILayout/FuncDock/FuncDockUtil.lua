...
typeFunction[BtnType.look_role] = function(targetId)
    -- 发送查看玩家请求，如果查到玩家数据流程正常，会发送LUA_EVENT_RESPONSE_LOOK_PLAYER_INFO事件
    -- 在FuncDock.main监听和处理LUA_EVENT_RESPONSE_LOOK_PLAYER_INFO这个事件
    -- 如果服务器不返回或者返回空数据则不触发LUA_EVENT_RESPONSE_LOOK_PLAYER_INFO事件
...
typeFunction[BtnType.chat] = function(targetId)
    ...
    FGUI:Open("Friend", "FriendPanel", data)
