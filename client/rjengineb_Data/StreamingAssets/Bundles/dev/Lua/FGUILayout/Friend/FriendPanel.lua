...
local FriendPanel = class("FriendPanel", BaseFGUILayout)
...
function FriendPanel:Enter(pageData)
    self:RegisterEvent()
...
function FriendPanel:UpdatePageItemRenderer(idx, item)
    local index = idx + 1
...
function FriendPanel:OnClickPage(eventData)
...
function FriendPanel:SelectPage(index)
...
function FriendPanel:UpdateList()
    local list = nil
    if self._selPage == FGUIDefine.FriendPage.Recent then  
...
function FriendPanel:SetNothingVisible()
...
function FriendPanel:UpdatePlayerOnline()
...
-- list friend
function FriendPanel:OnRendererFriend(idx, item)
...
function FriendPanel:OnClickPlayerInfo(context)
...
function FriendPanel:OnClickDeleteChat(context)
    local index = FGUI:GetIntData(context.sender.parent) + 1
    local data = self._showList[index]
    if not data then  
        return
...
local targetData = {}
function FriendPanel:OnClickItemFriend(context)
...
function FriendPanel:SetChatTarget(targetId, targetData, data)
...
-- list chat
function FriendPanel:UpdateChatView()
...
function FriendPanel:OnProviderChat(idx)
    ...
    if isSelf then
        return "ui://seywlb2rwmgrb"
    else
        return "ui://seywlb2rwmgr8"
    end
end

function FriendPanel:OnRendererChat(idx, item)
    if not self._messages or not next(self._messages) then 
        return 
    end
...
function FriendPanel:UpdateHistoryView(data)
    if not data then 
...
function FriendPanel:OnClickPositionLink(context)
    if not self._targetID then 
...
-- list emoj
function FriendPanel:OnRendererEmoj(idx, item)
    local index = idx + 1
...
function FriendPanel:OnClickItemEmoj(context)
    local listEmoj = FGUI:GetChild(self._ui.btn_emoj, "list_emoj")
    if listEmoj then 
...
-- evnet
function FriendPanel:OnSendMessage(eventData)
	FGUI:delayTouchEnabled(eventData.sender, FGUIDefine.DelayClickTime)
...
function FriendPanel:OnSendPostion(eventData)
	FGUI:delayTouchEnabled(eventData.sender, FGUIDefine.DelayClickTime)

    SL:RequestSendChatPosMsg(SLDefine.CHAT_CHANNEL.Private, self._targetID, self._targetData)
end

function FriendPanel:OpenAddFriendUI()
    FGUI:Open("Friend", "FriendApplyPanel", FGUIDefine.FriendOpreate.Add)
end

function FriendPanel:OnAddPrivateChatItem()
    self:UpdateChatView()
end

function FriendPanel:OnFriendListUpdate()
    if self._selPage ~= FGUIDefine.FriendPage.Friend then return end
    self:UpdateList()
    self:UpdateChatView()
end

function FriendPanel:OnRecentListUpdate()
    if self._selPage ~= FGUIDefine.FriendPage.Recent then return end
    self:UpdateList()
end

function FriendPanel:OnBlackListUpdate()
    if self._selPage ~= FGUIDefine.FriendPage.Black then return end
    self:UpdateList()
end

function FriendPanel:OnEnemyListUpdate()
    if self._selPage ~= FGUIDefine.FriendPage.Enemy then return end
    self:UpdateList()
end

-----------------------------------注册事件--------------------------------------
function FriendPanel:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_CHAT_ADD_PRIVATE_ITEM, "FriendPanel", handler(self, self.OnAddPrivateChatItem))
    SL:RegisterLUAEvent(LUA_EVENT_FRIEND_LIST_UPDATE, "FriendPanel", handler(self, self.OnFriendListUpdate))
    SL:RegisterLUAEvent(LUA_EVENT_RECENT_CHAT_LIST_UPDATE, "FriendPanel", handler(self, self.OnRecentListUpdate))
    SL:RegisterLUAEvent(LUA_EVENT_BLACK_LIST_UPDATE, "FriendPanel", handler(self, self.OnBlackListUpdate))
    SL:RegisterLUAEvent(LUA_EVENT_ENEMY_LIST_UPDATE, "FriendPanel", handler(self, self.OnEnemyListUpdate))

end

function FriendPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_CHAT_ADD_PRIVATE_ITEM, "FriendPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_FRIEND_LIST_UPDATE, "FriendPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_RECENT_CHAT_LIST_UPDATE, "FriendPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_BLACK_LIST_UPDATE, "FriendPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_ENEMY_LIST_UPDATE, "FriendPanel")
end
