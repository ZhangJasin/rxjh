...
local PCTeamTargetPanel = class("PCTeamTargetPanel", BaseFGUILayout)

function PCTeamTargetPanel:Create()
...
function PCTeamTargetPanel:Close()
	self.super.Close(self)
end

function PCTeamTargetPanel:InitData()
    self._targetData = {
...
function PCTeamTargetPanel:ItemRendererTarget(idx, item)
    local index = idx + 1
...
function PCTeamTargetPanel:OnClickBtnSelect(context)
    self:Close()
    SLBridge:onLUAEvent(LUA_EVENT_TEAM_TARGET_INFO)
end

return PCTeamTargetPanel