...
local GuildJoinList = class("GuildJoinList", BaseFGUILayout)
local ItemShow = SL:RequireFile("FGUILayout/Item/ItemShow")

function GuildJoinList:Create()
...
function GuildJoinList:Exit()
	SL:ComponentDetach(SLDefine.SUIComponentTable.GuildJoinList)

	self:RemoveEvent()
end

function GuildJoinList:Close()
	self.super.Close(self)
end
-- 点击创建门派
function GuildJoinList:OnClickCreateGuild()
	FGUI:Open("Guild", "GuildCreatePopup")
end

-- 加入公会列表Item显示
function GuildJoinList:OnJoinItemRenderer(idx, item)
...
-- 点击加入公会列表Item事件
function GuildJoinList:OnClickJoinItemEvent(context)
	local idx = FGUI:GList_getSelectedIndex(self._ui.list_join) + 1
	self:SelectJoinItem(idx)
end

-- 点击加入公会列表Item上的 加入按钮 事件
function GuildJoinList:OnClickJoinButtonEvent(eventData)
    FGUI:delayTouchEnabled(eventData.sender, FGUIDefine.DelayClickTime)
...
-- 一键申请
function GuildJoinList:OnClickQuickJoinEvent()
	SL:RequestGuildAutoJoin()
	FGUI:GButton_setBright(self._ui.btn_quick_join, false)
end

-- 搜素
function GuildJoinList:OnClickSearchEvent()
...
function GuildJoinList:SelectJoinItem(index)
...
function GuildJoinList:GetEvildStr(evild)
...
function GuildJoinList:OnRefreshGuildList(listData)
...
function GuildJoinList:RegisterEvent()
	SL:RegisterLUAEvent(LUA_EVENT_GUILD_LIST, "GuildJoinList", handler(self, self.OnRefreshGuildList))
end

function GuildJoinList:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_GUILD_LIST, "GuildJoinList")
end

return GuildJoinList