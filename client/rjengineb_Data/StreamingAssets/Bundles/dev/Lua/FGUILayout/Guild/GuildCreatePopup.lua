...
local GuildCreatePopup = class("GuildCreatePopup", BaseFGUILayout)
local ItemShow = SL:RequireFile("FGUILayout/Item/ItemShow")

function GuildCreatePopup:Create()
...
function GuildCreatePopup:Enter()
	self._isValid = true
    self:RegisterEvent()
...
function GuildCreatePopup:Exit()
	self:RemoveEvent()
	self._isValid = false
end

function GuildCreatePopup:Close()
	self.super.Close(self)
end

function GuildCreatePopup:InitM2RandGuildName()
	-- 请求随机行会名
...
-- 创建行会点击事件
function GuildCreatePopup:OnClickCreateGuildEvent(eventData)
...
-- 创建行会响应
function GuildCreatePopup:OnGuildCreateRes()
    -- 关闭行会创建界面
...
-- 创建条件列表刷新
function GuildCreatePopup:OnConditionItemRenderer(idx, item)
...
function GuildCreatePopup:RegisterEvent()
	SL:RegisterLUAEvent(LUA_EVENT_GUILD_CREATE, "GuildCreatePopup", handler(self, self.OnGuildCreateRes))
end

function GuildCreatePopup:RemoveEvent()
	SL:UnRegisterLUAEvent(LUA_EVENT_GUILD_CREATE, "GuildCreatePopup")
end
