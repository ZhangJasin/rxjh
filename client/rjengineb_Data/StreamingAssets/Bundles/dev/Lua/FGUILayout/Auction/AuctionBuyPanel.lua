...
local AuctionBuyPanel = class("AuctionBuyPanel", BaseFGUILayout)
...
function AuctionBuyPanel:Create()
...
function AuctionBuyPanel:InitData()
...
-- 开启定时器
function AuctionBuyPanel:StartTimer()
    self:EndTimer()
    self._sendTimer = SL:Schedule(handler(self,self.SendHandler),0.2)
end

-- 定时器句柄
function AuctionBuyPanel:SendHandler()
...
-- 关闭定时器
function AuctionBuyPanel:EndTimer()
...
-- 清除发送缓存
function AuctionBuyPanel:ClearSendCache()
    self._reqData = nil
end

function AuctionBuyPanel:CleanCache()
...
function AuctionBuyPanel:GetAllFGuiData()
...
function AuctionBuyPanel:InitClickEvent()
...
function AuctionBuyPanel:BtnJingJiaClicked()
...
function AuctionBuyPanel:BtnBuyClicked()
	if self._selectItemIndex == -1 then
		return
	end
...
function AuctionBuyPanel:BtnSearchClicked()
    self.ctrl_isSearchByKeyWord.selectedIndex = 0
    self._curPage = 1
    self:RequestSearchPaiMai()
end

function AuctionBuyPanel:BtnSearchSwitch()
...
-- 格式化页码
function AuctionBuyPanel:formatPageNums(num)
...
function AuctionBuyPanel:RefreshPageShow()
...
-- 品质选择发生变化
function AuctionBuyPanel:CBXAllGradeChanged()
...
-- 阵营过滤发生变化
function AuctionBuyPanel:CBXAllCampChanged()
...
-- 等级过滤器发生变化
function AuctionBuyPanel:CBXAllLevelChanged()
...
function AuctionBuyPanel:InitUI()
    -- 初始化树形结构
    ...
    for _key,_value in ipairs(self._treeData.tree) do
        ...
        local parent = FGUI:GTreeNode_Create("ui://Auction/item_listleft_parent",isHasChild)
        ...
        if _value and isHasChild then
            for _nPage,_value_ in pairs(_value) do
                local child = FGUI:GTreeNode_Create("ui://Auction/item_listleft_child",false)
...
function AuctionBuyPanel:CellClicked(context)
...
function AuctionBuyPanel:SelectCell(idx)
...
function AuctionBuyPanel:ListViewCellsItemRenderer(idx,cell)
    local index = idx + 1
    ...
    if data then
        ...
        if not string.isNullOrEmpty(data.currname) then
            ...
            FGUI:GTextField_setText(text_status,GET_STRING(30000204))
        end
...
function AuctionBuyPanel:Enter()
    self:StartTimer()
    self:RegisterEvent()
...
function AuctionBuyPanel:BtnNextClicked()
    self._curPage = self:formatPageNums(self._curPage + 1 )
    self:RequestPaiMaiPage()
    self:RequestSearchPaiMai()
end

function AuctionBuyPanel:BtnPrevClicked()
    self._curPage = self:formatPageNums(self._curPage - 1 )
    self:RequestPaiMaiPage()
    self:RequestSearchPaiMai()
end

function AuctionBuyPanel:BtnPrevFastClicked()
    self._curPage = self._minPage
    self:RequestPaiMaiPage()
    self:RequestSearchPaiMai()
end

function AuctionBuyPanel:BtnNextFastClicked()
...
-- 刷新列表
function AuctionBuyPanel:RefreshPaiMaiList()
...
-- 刷新当前页的数据
function AuctionBuyPanel:RefreshData(curPage,totalPage)
    if curPage then
...
-- 请求数据
function AuctionBuyPanel:RequestPaiMaiPage()
...
function AuctionBuyPanel:RequestSearchPaiMai()
...
function AuctionBuyPanel:BuyTips(recog)
    if not recog then
        return
    end
    if recog == -1 then
...
function AuctionBuyPanel:PlayerLoadingActionBySwitch(open)
...
function AuctionBuyPanel:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_PAGE_RES, "AuctionBuyPanel", handler(self,self.RefreshData))
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_PAGE_UPDATE, "AuctionBuyPanel", handler(self,self.RefreshData))
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_BUY_RESULT, "AuctionBuyPanel", handler(self,self.BuyTips))
    SL:RegisterLUAEvent(LUA_EVENT_STALL_BUY_FAIL_TIPS, "AuctionBuyPanel", handler(self,self.BuyTips))
    SL:RegisterLUAEvent(LUA_EVENT_MY_AUCTION_LOADING_UPDATE, "AuctionBuyPanel", handler(self, self.PlayerLoadingActionBySwitch))
end

function AuctionBuyPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_PAGE_RES, "AuctionBuyPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_PAGE_UPDATE, "AuctionBuyPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_BUY_RESULT, "AuctionBuyPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_STALL_BUY_FAIL_TIPS, "AuctionBuyPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_MY_AUCTION_LOADING_UPDATE, "AuctionBuyPanel")
end

return AuctionBuyPanel
