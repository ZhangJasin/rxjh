...
local TipConsignmentPanel = class("TipConsignmentPanel", BaseFGUILayout)
...
function TipConsignmentPanel:InitUI()
...
function TipConsignmentPanel:RefreshTabList()
    FGUI:GList_setNumItems(self.list_tab,#self.arrayNameTable)
end
-- 初始化数据
function TipConsignmentPanel:InitData()
...
function TipConsignmentPanel:GetAllFGuiData()
    -- 关闭按钮
...
function TipConsignmentPanel:InitClickEvent()
    ...
    FGUI:setOnFocusIn(self.input_jishou_count,handler(self,self.InputJiShouCountFoucsIn))
...
function TipConsignmentPanel:TabItemRender(idx,item)
    local index = idx + 1
...
function TipConsignmentPanel:TabItemClicked(contextData)
...
-- 过滤数据
function TipConsignmentPanel:TabFilterData()
...
function TipConsignmentPanel:ClickTabCellEvent(idx)
    self._cur_tab_index = idx
    self:RefreshTabList()
    self:RefreshCanPaiMaiData()
end

function TipConsignmentPanel:BagItemRender(idx,item)
    local index = idx + 1
...
function TipConsignmentPanel:BagItemClicked(contextData)
...
-- bagCell点击
function TipConsignmentPanel:ClickBagCellEvent(index)
    self._selected_item_index = index
    self:RefreshLeftPanelInfo()
end

-- 左边面板信息
function TipConsignmentPanel:RefreshLeftPanelInfo()
...
-- 刷新货币图标显示
function TipConsignmentPanel:RefreshMoneyItemShow(moneyData)
...
-- 刷新底部最低寄售底价
function TipConsignmentPanel:RefreshTipMinJiShou(moneyData)
...
-- 刷新寄售底价显示
function TipConsignmentPanel:RefreshJiShouDiJia()
    FGUI:GTextInput_setText(self.input_jsdj,self._cur_jishou_diJia)
end

-- 刷新一口价显示
function TipConsignmentPanel:RefreshOncePrice()
    FGUI:GTextInput_setText(self.input_once_price, self._cur_once_price)
end

function TipConsignmentPanel:InputJiShouCountEndInput(input)
...
-- 点击修改
function TipConsignmentPanel:InputJiShouCountFoucsIn(context)
    local bagData = self._can_paiMai_data[self._selected_item_index]
    if bagData then
        local data = {}
        data.title = GET_STRING(90010006)
        data.maxNum = bagData.BagData.OverLap
...
-- 寄售底价输入完毕
function TipConsignmentPanel:InputJiShouDiJiaOnFocusOut(context)
...
-- 一口价刷新
function TipConsignmentPanel:InputOncePriceOnFocusOut(context)
    local content = FGUI:GTextInput_getText(context.sender)
    if string.isNullOrEmpty(content) then
        self:RefreshOncePrice()
        return
...
function TipConsignmentPanel:CleanCache()
    if self._cur_selected_item then
        ItemUtil:ItemShow_Release(self._cur_selected_item)
    end
...
function TipConsignmentPanel:Destory()
    self:CleanCache()
end

function TipConsignmentPanel:RefreshJiShouCount()
    FGUI:GTextInput_setText(self.input_jishou_count,self._cur_jishou_count)
end

-- 刷新背包
function TipConsignmentPanel:RefreshCanPaiMaiData()
...
function TipConsignmentPanel:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_MY_AUCTION_ADD_SELL_SUCC, "TipConsignmentPanel", handler(self, self.RefreshCanPaiMaiData))
end

function TipConsignmentPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_MY_AUCTION_ADD_SELL_SUCC, "TipConsignmentPanel")
end

function TipConsignmentPanel:Enter()
    self:RegisterEvent()
    self:RefreshCanPaiMaiData()
end

function TipConsignmentPanel:Exit()
    self:RemoveEvent()
end

-- 寄售
function TipConsignmentPanel:BtnJiShouClicked()
    ...
    if data then
        ...
        FGUI:Open("Auction","TipJiShouPanel",data)
    end
end
...
-- 减一数量
function TipConsignmentPanel:BtnMinusClicked()
...
-- 加一数量
function TipConsignmentPanel:BtnAddClicked()
    self._cur_jishou_count = self._cur_jishou_count + 1
    local data = self._can_paiMai_data[self._selected_item_index]
    if not data then
        return
...
-- 最大值数量
function TipConsignmentPanel:BtnMaxClicked()
    local data = self._can_paiMai_data[self._selected_item_index]
    if not data then
        return
    end
...
function TipConsignmentPanel:OnClose()
    self.super.Close(self)
end

return TipConsignmentPanel