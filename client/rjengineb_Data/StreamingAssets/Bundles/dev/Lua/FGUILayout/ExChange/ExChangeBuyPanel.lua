...
local ExChangeBuyPanel = class("ExChangeBuyPanel", BaseFGUILayout)
local ItemUtil = SL:RequireFile("FGUILayout/Item/ItemUtil")

--- 界面被创建时调用
function ExChangeBuyPanel:Create()
...
function ExChangeBuyPanel:GetAllFGuiData()
...
function ExChangeBuyPanel:InitClickEvent()
...
function ExChangeBuyPanel:InitData()
	self._itemType = -1
	self._itemShowList = {}
...
function ExChangeBuyPanel:InitView()
	self:InitListFilter()
end

-- 刷新过滤结果
function ExChangeBuyPanel:InitListFilter()
	...
	for _key, _value in ipairs(self._treeData.tree) do
		...
		local parent = FGUI:GTreeNode_Create("ui://ExChange/item_listleft_parent", isHasChild)
		...
		if _value and isHasChild then
			for _nPage, _value_ in pairs(_value) do
				local child = FGUI:GTreeNode_Create("ui://ExChange/item_listleft_child", false)
...
function ExChangeBuyPanel:InputSearchChanged()
...
function ExChangeBuyPanel:ComBoxMoneyFilter()
...
function ExChangeBuyPanel:CheckMoneyFilter(costID)
	if not costID then
		return false
	end
...
-- 刷新combox显示
function ExChangeBuyPanel:RefreshItemMoneyFilter()
	if self._itemType ~= 1 then
		return
	end
...
function ExChangeBuyPanel:ClearData()
	self._itemType = -1
	self._nPage = self._groupSets[1]
	self._curReqItemIndex = nil
end

function ExChangeBuyPanel:ListDataItemRender(idx, item)
...
-- 购买物品
function ExChangeBuyPanel:BuyItemData(data)
	local enterData = {}
	...
	FGUI:Open("ExChange", "ExChangeBuyDialog", enterData)
end

-- 清理cache
function ExChangeBuyPanel:CleanCache()
...
-- 刷新按钮点击
function ExChangeBuyPanel:BtnrefreshClicked()
...
-- 搜索
function ExChangeBuyPanel:BtnSearchClicked()
	self:SearchData()
end

-- 按钮返回点击
function ExChangeBuyPanel:BtnReturnClicked()
...
-- 搜索数据
function ExChangeBuyPanel:SearchData()
...
function ExChangeBuyPanel:SearchFilter(name)
	self.keyword = FGUI:GTextInput_getText(self.input_search)
	-- 搜索内容是否合法
	if string.isNullOrEmpty(self.keyword) then
		return true
...
-- 过滤数据
function ExChangeBuyPanel:FilterData()
	self._showDataList = {}
...
-- 刷新列表
function ExChangeBuyPanel:RefreshListView()
...
-- 清除搜索
function ExChangeBuyPanel:BtnClearSearchClicked()
...
function ExChangeBuyPanel:BtnCollectionClicked()
	self:RefreshCollectData()
end

--- 界面打开时调用
function ExChangeBuyPanel:Enter(data)
	self:StartTimer()
	self:RegisterEvent()
	self:InitUI()
	self:RequestExCollectItem(2)
...
-- 初始化UI显示
function ExChangeBuyPanel:InitUI()
	FGUI:GTextInput_setText(self.input_search,"")
	FGUI:Controller_setSelectedIndex(self.ctrl_isHaveSearchKey,1)
...
--- 界面关闭时调用
function ExChangeBuyPanel:Exit()
...
-- 开启定时器
function ExChangeBuyPanel:StartTimer()
	self:EndTimer()
	self._sendTimer = SL:Schedule(handler(self,self.SendHandler),0.2)
end

-- 请求句柄
function ExChangeBuyPanel:SendHandler()
...
-- 关闭定时器
function ExChangeBuyPanel:EndTimer()
...
-- 请求物品汇总信息
function ExChangeBuyPanel:RequestItemSummyInfo()
...
-- 请求某个物品的所有挂单
function ExChangeBuyPanel:RequestExItemAllOrder()
...
-- 收藏相关操作
function ExChangeBuyPanel:RequestExCollectItem(opt,itemID)
	if not opt then
		return
	end
...
function ExChangeBuyPanel:PushOptInQueue(callback,opt)
...
function ExChangeBuyPanel:PlayerLoadingActionBySwitch(open)
...
-- 更新汇总数据
function ExChangeBuyPanel:RefreshAllItemSummyData()
	self._itemType = 0
...
-- 更新物品所有的挂单数据
function ExChangeBuyPanel:RefreshAllItemOrderData()
	self._itemType = 1
...
-- 更新收藏数据
function ExChangeBuyPanel:RefreshCollectData()
	self._itemType = 2
...
function ExChangeBuyPanel:BuyTips(recog)
	if not recog then
		return
	end
	if recog == -1 then
...
function ExChangeBuyPanel:RegisterEvent()
	SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_SUM_DATA_UPDATE, "ExChangeBuyPanel", handler(self, self.RefreshAllItemSummyData))
	SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_ITEM_ALL_ORDER, "ExChangeBuyPanel", handler(self, self.RefreshAllItemOrderData))
	SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_COLLECTS_UPDATE, "ExChangeBuyPanel", handler(self, self.RefreshCollectData))
	SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_BUY_FAIL_TIPS, "ExChangeBuyPanel", handler(self, self.BuyTips))
	SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_BUY_RESULT, "ExChangeBuyPanel", handler(self, self.BuyTips))
	SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_LOADING_UPDATE, "ExChangeBuyPanel", handler(self, self.PlayerLoadingActionBySwitch))
end

function ExChangeBuyPanel:UnRegisterEvent()
	SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_SUM_DATA_UPDATE, "ExChangeBuyPanel")
	SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_ITEM_ALL_ORDER, "ExChangeBuyPanel")
	SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_COLLECTS_UPDATE, "ExChangeBuyPanel")
	SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_BUY_FAIL_TIPS, "ExChangeBuyPanel")
	SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_BUY_RESULT, "ExChangeBuyPanel")
	SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_LOADING_UPDATE, "ExChangeBuyPanel")
end
