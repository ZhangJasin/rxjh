...
function Invitation:onClickBtnCZ()
    --师徒副本锁
    if self._isEntering then return end
    ...
    if nums == #self.waitingListData - 1 then
        SL:UnSchedule(self.dsq)
        InvitationUI.CCUI = self
        ssrMessage:sendmsgEx("MentorShip", "mapMoveAll", {
            users = finallyUserList,
            taskType = self.task
                .task_target_param
        })
...
function Invitation:yaoqing()
    local t_val = SL:GetValue("T", 96)
    local doneList = 0

    if t_val and t_val ~= "" then
        doneList = SL:JsonDecode(t_val)
    end

    local doneAppr = {}
    if doneList == 0 or type(doneList) ~= "table" then

    else
        --今日师徒副本进入情况
        doneAppr = doneList[tonumber(self.task.task_target_param)] or {}
    end
