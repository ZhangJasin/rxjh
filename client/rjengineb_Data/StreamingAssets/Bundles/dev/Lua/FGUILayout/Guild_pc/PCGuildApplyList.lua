...
local PCGuildApplyList = class("PCGuildApplyList", BaseFGUILayout)
...
function PCGuildApplyList:Enter()
    self:RegisterEvent()
...
function PCGuildApplyList:Exit()
	SL:ComponentDetach(SLDefine.SUIComponentTable.GuildApplyList)
	SL:DelBubbleTips(10)
	self:RemoveEvent()
end

function PCGuildApplyList:Close()
...
-- 全选
function PCGuildApplyList:OnClickSelectAll()
	self._isAllSelected = not self._isAllSelected
	self:SetAllSelect(self._isAllSelected)
end

function PCGuildApplyList:SetAllSelect(isSelect)
...
-- 点击申请列表项
function PCGuildApplyList:OnClickApplyItem(context)
...
-- 打开设置界面
function PCGuildApplyList:OpenSettingPanel()
...
function PCGuildApplyList:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_GUILD_APPLYLIST, "PCGuildApplyList", handler(self, self.RefreshApplyList))
end

function PCGuildApplyList:RemoveEvent()
	SL:UnRegisterLUAEvent(LUA_EVENT_GUILD_APPLYLIST, "PCGuildApplyList")
end

return PCGuildApplyList