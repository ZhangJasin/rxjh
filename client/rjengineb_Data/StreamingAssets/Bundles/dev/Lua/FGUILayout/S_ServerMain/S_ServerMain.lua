...
function S_ServerMain:InitModName()
	if self._ui.node_server_name then
		local modName = ""
		if global.isBoxLogin then
			local subModInfo = global.S_ServerMainManager:GetCurrentSubMod()
	
			if subModInfo and subModInfo.name then
				modName = subModInfo.name
			end
			FGUI:GButton_setTitle(self._ui.node_server_name, modName)
		else
			if global.isWindows then
				modName = global.L_GameEnvManager:GetEnvDataByKey("subModName") or ""
			else
				modName = global.L_NativeBridgeManager:GetNativeDataByKey("packageName") or ""
			end
			if string.len(modName) == 0 then
				FGUI:setVisible(self._ui.node_server_name, false)
			else
				FGUI:GButton_setTitle(self._ui.node_server_name, modName)
			end
		end
...
function S_ServerMain:UpdateSelectServer()
	...
	local new = ""
	local new_icon = FGUI:GetChild(self._ui.Btn_change_server, "icon_new")
	if selectedServer.state == 1 then
		if new_icon then
			FGUI:setVisible(new_icon, true)
		else
			new = SL:GetValue("I18N_STRING", 33)	
		end
	else
		if new_icon then
			FGUI:setVisible(new_icon, false)
		end
	end
...
function S_ServerMain:OnRoleInfoChange()
    self:OnRoleInfoGet()
end
...
function S_ServerMain:RegisterEvent()
	...
	SL:RegisterLUAEvent(LUA_EVENT_ROLEINFO_CHANGE, "ServerMain", handler(self, self.OnRoleInfoChange))
...
function S_ServerMain:RemoveEvent()
	...
	SL:UnRegisterLUAEvent(LUA_EVENT_ROLEINFO_CHANGE, "ServerMain")
end
