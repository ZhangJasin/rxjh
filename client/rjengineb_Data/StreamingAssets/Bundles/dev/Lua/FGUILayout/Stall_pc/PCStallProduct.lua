...
local PCStallProduct = class("PCStallProduct", BaseFGUILayout)
...
function PCStallProduct:Create()
	...
	self.handler_onDropEvent = handler(self, self.OnDropEvent)
...
function PCStallProduct:Close()
	self.super.Close(self)
end

function PCStallProduct:CloseStallShop()
	local dialogData = {}
...
function PCStallProduct:OnProductItemRenderer(idx, item)
	local data = self._product_data[idx + 1]
	FGUI:SetIntData(item, idx)
	local empty = FGUI:GetChild(item, "empty")
	FGUI:setOnDropEvent(item, self.handler_onDropEvent)
...
function PCStallProduct:OnDropEvent(eventData)
	self._clickDelay = true
	SL:ScheduleOnce(handler(self, self.OnDelayClickEnd, nil, true), 0.1)
	if not eventData then return end
	if not eventData.data then return end
	local makeIndex = eventData.data.makeIndex
	...
	if eventData.data.from and eventData.data.from == ItemFrom.BAG then
		...
		self:PutOnItem(itemData)
	end
end

function PCStallProduct:OnDelayClickEnd()
	self._clickDelay = false
end

function PCStallProduct:OnClickProductItem(context)
	if self._clickDelay then return end
...
-- 显示物品提示
function PCStallProduct:ShowItemTips(context)
...
-- 点击编辑店铺名按钮
function PCStallProduct:OnClickEditNameButton()
	local data = {}
...
-- 点击分享按钮
function PCStallProduct:OnClickShareButton()
	SL:RequestSendTradeMsg(CHANNEL.Trade, self._data.Name, self._data.Userid)
end

-- 上架
function PCStallProduct:PutOnItem(itemData)
	if itemData._itemData then
	...
	if isOnStallArea then
		FGUI:Open("Stall_pc", "PCStallShelf", itemData)
	else
		local data = {}
...
-- 上架物品成功
function PCStallProduct:OnPutOnSuccess(data, notTips)
	if not data then return end
...
-- 下架物品成功
function PCStallProduct:OnTakeOffSuccess(makeindex, notTips)
...
-- 打开下架界面
function PCStallProduct:OpenTakeOffPanel(data)
	if not data then return end
	local dialogData = {}
...
-- 购买物品
function PCStallProduct:OnBuyItem(data)
...
-- 物品变化
function PCStallProduct:OnItemChange(data)
...
-- 摆摊购买失败提示
function PCStallProduct:OnBuyFailTips(errorType)
	if errorType == -2 then
...
-- 点击收藏按钮
function PCStallProduct:OnClickCollectShop()
...
function PCStallProduct:OnCollectShop(userid, isCollected)
...
function PCStallProduct:RegisterEvent()
   	SL:RegisterLUAEvent(LUA_EVENT_STALL_PUT_ON_SUCCESS, "PCStallProduct", handler(self, self.OnPutOnSuccess))
	SL:RegisterLUAEvent(LUA_EVENT_STALL_REFRESH_PRODUCT, "PCStallProduct", handler(self, self.OnRefreshProductInfo))  
	SL:RegisterLUAEvent(LUA_EVENT_STALL_TAKE_OFF, "PCStallProduct", handler(self, self.OnTakeOffSuccess))
	SL:RegisterLUAEvent(LUA_EVENT_STALL_BUY_ITEM, "PCStallProduct", handler(self, self.OnBuyItem))
	SL:RegisterLUAEvent(LUA_EVENT_STALL_PRODUCT_CHANGE, "PCStallProduct", handler(self, self.OnItemChange))
	SL:RegisterLUAEvent(LUA_EVENT_STALL_BUY_FAIL_TIPS, "PCStallProduct", handler(self, self.OnBuyFailTips))
	SL:RegisterLUAEvent(LUA_EVENT_STALL_OPEN_SHELF, "PCStallProduct", handler(self, self.PutOnItem))
	SL:RegisterLUAEvent(LUA_EVENT_BAG_CELL_CLICK, "PCStallProduct", handler(self, self.PutOnItem))
...
function PCStallProduct:RemoveEvent()
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_PUT_ON_SUCCESS, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_REFRESH_PRODUCT, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_TAKE_OFF, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_BUY_ITEM, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_PRODUCT_CHANGE, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_BUY_FAIL_TIPS, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_OPEN_SHELF, "PCStallProduct")
	SL:UnRegisterLUAEvent(LUA_EVENT_BAG_CELL_CLICK, "PCStallProduct")
