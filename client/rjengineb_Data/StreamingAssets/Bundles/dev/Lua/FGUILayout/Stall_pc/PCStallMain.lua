...
local PCStallMain = class("PCStallMain", BaseFGUILayout)
-- 摆摊主界面
local countPerPage = 6 -- 每页摊位数
function PCStallMain:Create()
...
function PCStallMain:Exit()
	...
	FGUI:Close("Stall_pc", "PCStallProduct")
	FGUI:Close("Bag_pc", "PCSimpleBagPanel")
	FGUIFunction:HideTopCurrency()
end

function PCStallMain:Close()
	self.super.Close(self)
end

-- 搜索
function PCStallMain:OnSearchStall()
...
-- 打开购物车
function PCStallMain:OnClickShoppingCart()
	FGUI:Open("Stall_pc", "PCStallShoppingCart")
end


-- 点击收藏按钮
function PCStallMain:OnClickFilterCollect()
...
-- 跳转到某页
function PCStallMain:GotoPage(page)
	self._curPage = page
...
-- 顶部UI显示控制
function PCStallMain:RefreshSelfInfoDisplay()
...
-- 刷新自身商铺信息
function PCStallMain:OnRefreshSelfShop()
	self._my_stall_data = SL:GetValue("STALL_MY_DATA")
	self:RefreshSelfInfoDisplay()
end

-- 刷新摊位列表数据
function PCStallMain:OnRefreshShopList(data, isFlipQuery, totalPage)
...
function PCStallMain:ShowShopListByCurPage()
	self._shop_list = {}
	if self._isFlipQuery then
	...
	FGUI:GTextField_setText(self._ui.text_page, string.format(GET_STRING(90010001), self._curPage, self._maxPageCount))
	if FGUI:CheckOpen("Stall_pc", "PCStallProduct") then
...
function PCStallMain:OnStallItemRenderer(idx, item)
...
-- 打开创建摆摊界面
function PCStallMain:OpenCreateStallPanel()
...
-- 是否在摆摊区域
function PCStallMain:IsOnStallArea()	
	return SL:GetValue("STALL_IS_ON_SHOP_AREA")
end

-- 当前位置是否可以摆摊
function PCStallMain:IsCanStall()
	-- 当前分线是否可以摆摊
...
-- 前往摆摊区域
function PCStallMain:MoveToStallArea()	
...
-- 点击浏览摊位
function PCStallMain:OnClickBrowseStall(context)
...
-- 点击店铺收藏按钮
function PCStallMain:OnClickCollectShop(context)
	local btn = context.sender
	local idx = FGUI:GetIntData(FGUI:GetParent(btn))
	local shopData = self._shop_list[idx + 1]
...
-- 切换浏览摊位
function PCStallMain:SwitchBrowseStall(isNext)
...
function PCStallMain:OnCollectShop(userid, isCollected)
...
-- 刷新当前页面摊位信息
function PCStallMain:RefreshCurrentPage()
	self:GotoPage(self._curPage)
end


function PCStallMain:RegisterEvent()
	SL:RegisterLUAEvent(LUA_EVENT_STALL_REFRESH_SHOP_LIST, "PCStallMain", handler(self, self.OnRefreshShopList))	--摊位列表刷新
	SL:RegisterLUAEvent(LUA_EVENT_STALL_REFRESH_SELF_SHOP, "PCStallMain", handler(self, self.OnRefreshSelfShop))	--刷新自身摊位数据
	SL:RegisterLUAEvent(LUA_EVENT_STALL_BROWSE_SWITCH, "PCStallMain", handler(self, self.SwitchBrowseStall))	--切换浏览摊位
	SL:RegisterLUAEvent(LUA_EVENT_STALL_COLLECT_SHOP, "PCStallMain", handler(self, self.OnCollectShop))	--收藏店铺
	SL:RegisterLUAEvent(LUA_EVENT_STALL_REFRESH_PAGE, "PCStallMain", handler(self, self.RefreshCurrentPage))	--刷新当前页面摊位信息
end

function PCStallMain:RemoveEvent()
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_REFRESH_SHOP_LIST, "PCStallMain")
    SL:UnRegisterLUAEvent(LUA_EVENT_STALL_REFRESH_SELF_SHOP, "PCStallMain")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_BROWSE_SWITCH, "PCStallMain")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_COLLECT_SHOP, "PCStallMain")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_REFRESH_PAGE, "PCStallMain")
end
