...
local PCStallShoppingCart = class("PCStallShoppingCart", BaseFGUILayout)
local ItemUtil = SL:RequireFile("FGUILayout/Item/ItemUtil")


function PCStallShoppingCart:Create()
...
function PCStallShoppingCart:Enter()
	self:RegisterEvent()
	self:RefreshDisplay()
	self._buyFailItems = {}	-- 重新打开清空失败商品列表
end

function PCStallShoppingCart:RefreshDisplay()
...
function PCStallShoppingCart:OnItemRenderer(idx, item)
	...
	local input_buy_count = FGUI:GetChild(item, "text_buy_count")
	FGUI:GTextInput_setOnChanged(input_buy_count, handler(self, self.OnCountValueChanged))
	FGUI:setOnFocusOut(input_buy_count, handler(self, self.OnCountValueFocusOut))
...
-- 点击查看摊位主
function PCStallShoppingCart:OnClickOwnerInfo(context)
...
-- 显示物品提示
function PCStallShoppingCart:ShowItemTips(context)
	FGUI:EventContext_stopPropagation(context)
	local idx = FGUI:GetIntData(FGUI:GetParent(context.sender))
	local data = self._data[idx + 1]
	local itemData = SL:GetValue("ITEM_DATA", data.index)
...
-- 选中/取消物品
function PCStallShoppingCart:OnItemSelectedChanged(context)
...
-- 物品购买数量输入
function PCStallShoppingCart:OnCountValueChanged(context)
	FGUI:EventContext_stopPropagation(context)
	...
	if str and str ~= "" then
		...
		if num > cartData.maxcount then
			num = cartData.maxcount
		end
		cartData.buycount = num
		if not num then
			self:SetBuyCountText(parent, cartData.buycount, cartData.price)
		else
			self:SetBuyCountText(parent, num, cartData.price)
		end
	end
...
-- 物品购买数量结束输入
function PCStallShoppingCart:OnCountValueFocusOut(context)
	local str = FGUI:GTextInput_getText(context.sender)
	if str and str == "" then
		local parent = FGUI:GetParent(context.sender)
		local idx = FGUI:GetIntData(parent)
		local cartData = self._data[idx + 1]
		cartData.buycount = 1
		self:SetBuyCountText(parent, 1, cartData.price)
	end
end

-- 打开修改数量界面
function PCStallShoppingCart:OpenEditCountPanel(context)
	FGUI:EventContext_stopPropagation(context)
	local parent = FGUI:GetParent(context.sender)
	local idx = FGUI:GetIntData(parent)
	local cartData = self._data[idx + 1]
...
-- 设置最大购买数量
function PCStallShoppingCart:SetMaxBuyCount(count, context)
	FGUI:EventContext_stopPropagation(context)
	local parent = FGUI:GetParent(context.sender)
	local idx = FGUI:GetIntData(parent)
	local data = self._data[idx + 1]
...
function PCStallShoppingCart:SetBuyCountText(parent, count, price)
...
-- 将物品从购物车移除
function PCStallShoppingCart:RemoveItem(makeindex, context)
	FGUI:EventContext_stopPropagation(context)
...
-- 更新合计
function PCStallShoppingCart:RefreshTotalMoney()
...
function PCStallShoppingCart:OnTotalMoneyListRenderer(idx, item)
...
-- 点击结算
function PCStallShoppingCart:OnClickBuyAll()
...
function PCStallShoppingCart:Exit()
	self:RemoveEvent()
	-- 保存购物车信息，保存选中状态和购物数量
	local data = {}
...
function PCStallShoppingCart:Close()
	self.super.Close(self)
end

function PCStallShoppingCart:OnBuyFailTips(errorType, makeindex)
	local errorStr = ""
	if errorType == -2 then
...
function PCStallShoppingCart:RegisterEvent()
	SL:RegisterLUAEvent(LUA_EVENT_STALL_REFRESH_CART, "PCStallShoppingCart", handler(self, self.RefreshDisplay))
	SL:RegisterLUAEvent(LUA_EVENT_STALL_BUY_FAIL_TIPS, "PCStallShoppingCart", handler(self, self.OnBuyFailTips))
end

function PCStallShoppingCart:RemoveEvent()
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_REFRESH_CART, "PCStallShoppingCart")
	SL:UnRegisterLUAEvent(LUA_EVENT_STALL_BUY_FAIL_TIPS, "PCStallShoppingCart")
end

return PCStallShoppingCart