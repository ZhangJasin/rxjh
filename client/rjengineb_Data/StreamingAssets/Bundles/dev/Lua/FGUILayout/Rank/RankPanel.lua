...
local RankPanel = class("RankPanel", BaseFGUILayout)
...
function RankPanel:InitEvent()
	...
	for i, group in ipairs(self._rankGroup) do 
		-- parent
        self.parent[i] = FGUI:GTreeNode_Create("ui://bzmikhguitlam", true)
		...
		for j, menu in ipairs(group) do 
			...
			self.child[i][j] = FGUI:GTreeNode_Create("ui://bzmikhguid4uy", false)
...
function RankPanel:SelectGroup(selectIdx)
	self._selectUserID = nil
	self._selectGroupID = selectIdx
end

function RankPanel:SelectMenu(selectIdx)
	self._selectUserID = nil
	self._selectMenuID = self._rankGroup[self._selectGroupID][selectIdx].nId
end

function RankPanel:SelectType()
	self:RefreshTypeList()
	self:RefreshTextTitleList()
end

-- rank type
function RankPanel:RefreshTypeList()
...
function RankPanel:ItemRendererType(idx, item)
	local index = idx + 1
...
-- text title
function RankPanel:RefreshTextTitleList()
	if not self._selectGroupID then 
		return 
	end 
	...
	if config.ValueName then 
		local info = {id = 2, name = config.ValueName, key = "ValueName"}
		table.insert(self._rankInfo, info)
	end 
...
function RankPanel:ItemRendererTitle(idx, item)
	local index = idx + 1
...
function RankPanel:OnResponseRankData(menuID)
...
function RankPanel:ItemRendererRank(idx, item)
	...
	if rank <= 3 then     
		...
		FGUI:GLoader_setUrl(img_rank, string.format("ui://Rank/rank_icon%s", rank))
	else 
	...
	FGUI:GList_itemRenderer(list_value, function(idx, item)
		...
		if key == "Name" then     
			FGUI:GTextField_setText(text_value, data.Name)
		elseif key == "ValueName" then 
			FGUI:GTextField_setText(text_value, data.Value)
...
function RankPanel:ShowMyRank()
	...
	FGUI:GList_itemRenderer(self._ui.list_self, function(idx, item)
		...
		elseif key == "ValueName" then 
			FGUI:GTextField_setText(text_value, myData.Value)
		elseif key == "GuildName" then 
...
function RankPanel:OnClickBtnArrow(context)
...
function RankPanel:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_RANK_DATA_UPDATE, "RankPanel", handler(self, self.OnResponseRankData))
end

function RankPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_RANK_DATA_UPDATE, "RankPanel")
end
