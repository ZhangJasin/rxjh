...
function FashionSystemPanl:UpdateListItemSelect()
    if self.LeftFashion and self.LeftFashion.itemlist then
        -- 先滚动到选中的项（确保目标项进入可视范围）
        if self.leftindex and self.leftindex > 0 then
            FGUI:GList_ScrollToView(self.LeftFashion.itemlist, self.leftindex - 1, true, false)
        end
        
        -- 延迟一帧后刷新虚拟列表，让 itemRenderer 重新设置选中状态
        SL:schedule(self._ui.bg, function()
            -- 重新设置 numItems 会触发 itemRenderer 重新渲染可见项
            FGUI:GList_setNumItems(self.LeftFashion.itemlist, FGUI:GList_getNumItems(self.LeftFashion.itemlist))
        end, 0.1, 1)
    end
end
