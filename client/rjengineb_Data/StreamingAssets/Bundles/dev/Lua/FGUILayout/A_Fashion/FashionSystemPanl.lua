...
function FashionSystemPanl:Create()
    ...
    -- 获取右侧页签按钮的Controller，用于正确切换选中状态
    self._rightPageControllers = {}
    for i=1,#_fashion_right_page do
        self._rightPageControllers[i] = FGUI:getController(self._ui[''.._fashion_right_page[i]], "isSelected")
    end
    
    -- 更多时装列表按钮
    if self._ui.open_more then
        FGUI:setOnClickEvent(self._ui.open_more, function()
            self:OpenMoreList()
        end)
    end
    if self._ui.close_more then
        FGUI:setOnClickEvent(self._ui.close_more, function()
            self:CloseMoreList()
        end)
    end
end
...
-- 未激活时装列表项点击事件：检查背包道具并请求激活
function FashionSystemPanl:OnClickUnactiveItem(type, configIndex, fashionID)
    local playsex = SL:GetValue("SEX") + 1          -- 玩家性别 1男2女
    local playjob = SL:GetValue("JOB")              -- 玩家职业 1弓手,2枪客,3刺客,4医生,5刀客,6剑客
    ...
    if type == _FashionType.PiFeng then
        equipID = fashion_pifeng_data[configIndex]['sex_type'][playsex] or 0
    elseif type == _FashionType.HuanWan then
        equipID = fashion_huanwu_data[configIndex]['job_type'][playjob] or 0
    elseif type == _FashionType.TouShi then
        equipID = fashion_toushi_data[configIndex]['sex_type'][playsex] or 0
    end
    
    if equipID <= 0 then
        SL:ShowSystemTips("时装数据错误！")
        return
    end
    
    -- 加载fashion_jihuo配置表，反向查找道具ID
    local fashion_jihuo = require("game_config/cfgcsv/fashion_jihuo")
    local itemID = 0
    ...
    if itemID <= 0 then
        SL:ShowSystemTips("未找到对应的激活道具！")
        return
    end
    
    -- 检查背包中是否有该道具
    local count = SL:GetValue("ITEM_COUNT", itemID) or 0
    if count <= 0 then
        local itemData = SL:GetValue("ITEM_DATA", itemID)
        if itemData and itemData.Name then
            SL:ShowSystemTips("背包中需要" .. itemData.Name .. "！")
        else
            SL:ShowSystemTips("背包中需要对应的激活道具！")
        end
        return
    end
    
    -- 背包中有道具，请求服务端激活
    SL:ShowSystemTips("正在激活...")
    FashionSystemData:RequestJiHuo({type, configIndex})
end

function FashionSystemPanl:OpenMoreList()                    -- 打开更多时装列表
    if not self._ui.more_list then
        SL:ShowSystemTips("more_list控件未找到")
        return
    end
    
    -- 加载more_list组件
    self._moreListObj = FGUI:ui_delegate(self._ui.more_list)
    
    -- 设置可见
    FGUI:setVisible(self._ui.more_list, true)
    if self._ui.open_more then
        FGUI:setVisible(self._ui.open_more, false)
    end
    if self._ui.close_more then
        FGUI:setVisible(self._ui.close_more, true)
    end
...
function FashionSystemPanl:CloseMoreList()                   -- 关闭更多时装列表
    if self._ui.more_list then
        FGUI:setVisible(self._ui.more_list, false)
    end
    if self._ui.close_more then
        FGUI:setVisible(self._ui.close_more, false)
    end
    -- 调用 CheckMoreButton 来决定 open_more 是否显示（而不是无条件显示）
    self:CheckMoreButton()
end

function FashionSystemPanl:RenderMoreList()                  -- 渲染更多时装列表
    -- 使用 OpenMoreList 中加载的 more_list 组件
    if not self._moreListObj then
        print("[RenderMoreList] _moreListObj 为空")
        return
    end

    -- 从加载的组件中获取 fashions 列表
    local moreList = self._moreListObj.fashions

    if not moreList then
        print("[RenderMoreList] moreList 为空")
        return
    end

    -- 将 leftFashionList 的所有数据放入 more_list
    self._moreListData = {}
    for i=1,#self.leftFashionList do
        table.insert(self._moreListData, self.leftFashionList[i])
    end

    print(string.format("[RenderMoreList] leftFashionList总数=%d, moreList数据数=%d", #self.leftFashionList, #self._moreListData))

    -- 清空列表
    FGUI:GList_removeChildrenToPool(moreList)

    -- 通过循环手动添加列表项（适用于 flow_hz 流式布局）
    print("[RenderMoreList] 开始循环创建列表项")
    for i=1,#self._moreListData do
        local item = FGUI:GList_addItemFromPool(moreList)
        if item then
            print(string.format("[RenderMoreList] 成功创建第%d个列表项", i))
            -- 手动设置列表项的数据
            local itemRoot = FGUI:GetChild(item, "itemRoot")
            FGUI:SetIntData(itemRoot, i)

            local data = self._moreListData[i]
            if data then
                local fashionID = data[1]
                local itemData = SL:GetValue("ITEM_DATA", tonumber(fashionID))

                -- 获取选中控制器，高亮当前选中的项
                local selectController = FGUI:getController(item,"select")
                if selectController then
                    if self.leftindex and i == self.leftindex then
                        FGUI:Controller_setSelectedIndex(selectController, 1)  -- 选中状态
                    else
                        FGUI:Controller_setSelectedIndex(selectController, 0)  -- 正常状态
                    end
                end
                ...
                if title and itemData then
                    FGUI:GTextField_setText(title, ""..itemData.Name)
                    print(string.format("[RenderMoreList] 第%d项设置标题=%s", i, itemData.Name))
                end

                -- 使用数据层的数据判断是否激活
                local fashionData = FashionSystemData:GetState().FashionDate
                local activeController = FGUI:getController(item, "active")
                if activeController then
                    if fashionData and fashionData["".._FashionTypeKey[self.rightPage]] and fashionData["".._FashionTypeKey[self.rightPage]][""..fashionID] then
                        FGUI:Controller_setSelectedIndex(activeController, 1)  -- 已激活状态
                        -- 注释掉剩余时间显示逻辑
                        -- local sytime2 = FGUI:GetChild(item, "sytime2")
                        -- if sytime2 then
                        ...
                        --         if sytime > 0 then
                        --             local statestr = SL:SecondToHMS(sytime, true, false)
                        --             FGUI:GTextField_setText(sytime2, "剩余时间："..statestr)
                        --         else
                        --             FGUI:GTextField_setText(sytime2, "已激活")
                        --         end
                        --     else
                        --         FGUI:GTextField_setText(sytime2, "已激活")
                        --     end
                        ...
                        FGUI:Controller_setSelectedIndex(activeController, 0)  -- 未激活状态
                        -- 未激活的显示"未激活"（sytime，使用XML默认文字，不手动设置）
                    end
                end
            else
                print(string.format("[RenderMoreList] 第%d项数据为空", i))
            end
        else
            print(string.format("[RenderMoreList] 第%d项创建失败", i))
        end
    end

    -- 打印最终列表项数量
    local finalCount = FGUI:GList_getNumItems(moreList)
    print(string.format("[RenderMoreList] 最终列表项数量=%d", finalCount))

    -- 添加点击事件（逻辑与主列表相同，额外增加关闭 more_list 的步骤）
    FGUI:GList_addOnClickItemEvent(moreList, function(context)
        local item = context.data
        local itemRoot = FGUI:GetChild(item, "itemRoot")
        local index = FGUI:GetIntData(itemRoot)

        if self._moreListData[index] then
            local fashionID = self._moreListData[index][1]
            local configIndex = self._moreListData[index][3]
            local fashionData = FashionSystemData:GetState().FashionDate

            -- 判断是否已选中
            local isAlreadySelected = (self.leftindex == index)

            -- 判断是否未激活
            local isActive = fashionData and fashionData["".._FashionTypeKey[self.rightPage]]
                             and fashionData["".._FashionTypeKey[self.rightPage]][""..fashionID]

            if isAlreadySelected then
                -- 点击已选中项
                if not isActive then
                    -- 已选中但未激活：执行激活流程
                    self:OnClickUnactiveItem(self.rightPage, configIndex, fashionID)
                    -- 激活后刷新列表
                    self:RenderMoreList()
                else
                    -- 已选中且已激活：刷新右侧展示
...
function FashionSystemPanl:CheckMoreButton()                   -- 检查是否需要显示更多按钮
    -- 检查实际时装列表的数量（而不是配置表总数）
    print(string.format("[CheckMoreButton] rightPage=%d, leftFashionList数量=%d", self.rightPage, #self.leftFashionList))
    if #self.leftFashionList > 7 then
        FGUI:setVisible(self._ui.open_more, true)
        print("[CheckMoreButton] 显示更多按钮")
    else
        FGUI:setVisible(self._ui.open_more, false)
        print("[CheckMoreButton] 隐藏更多按钮")
    end
end

function FashionSystemPanl:ChangeRightPage()                -- 切换右侧页签
    for i=1,#_fashion_right_page do
        local btn = self._ui[''.._fashion_right_page[i]]
        local ctrl = self._rightPageControllers[i]
        if i == self.rightPage then
            FGUI:GButton_setTitleColor(btn, "#ffff00")
            if ctrl then
                FGUI:Controller_setSelectedIndex(ctrl, 0)  -- 选中状态
            end
        else
            FGUI:GButton_setTitleColor(btn, "#CCCCCC")
            if ctrl then
                FGUI:Controller_setSelectedIndex(ctrl, 1)  -- 未选中状态
            end
        end
    end
    self:GetFashinPageData()                                -- 获取当前页时装数据
    self:InitData()                                         -- 切换页签更新数据
    -- 直接选中列表第一项，不调用FireClick避免循环
    self.leftindex = 1
    self:UpdateListItemSelect()
    self:UpdateRoleModel()
    self:UpdateRightAttr()
...
function FashionSystemPanl:InitData()                       -- 切换页签更新数据
    ...
    FGUI:GList_addOnClickItemEvent(self.LeftFashion.itemlist, function(context)
        local index = FGUI:GList_getSelectedIndex(self.LeftFashion.itemlist) + 1
        if self.leftFashionList[index] then
            local fashionID = self.leftFashionList[index][1]
            local configIndex = self.leftFashionList[index][3]
            local fashionData = FashionSystemData:GetState().FashionDate
...
function FashionSystemPanl:ListViewCellsItemRenderer(idx,item)
    ...
    local selectController = FGUI:getController(item,"select")
    if selectController then
        if self.leftindex and idx+1 == self.leftindex then
            FGUI:Controller_setSelectedIndex(selectController, 1)  -- 选中状态
        else
            FGUI:Controller_setSelectedIndex(selectController, 0)  -- 正常状态
        end
    ...
    if self.leftFashionList[idx+1] then
        ...
        local modelID = self.leftFashionList[idx+1][2]
        local configIndex = self.leftFashionList[idx+1][3]
        local itemData = SL:GetValue("ITEM_DATA", tonumber(fashionID))
...
-- 更新列表项选中状态
function FashionSystemPanl:UpdateListItemSelect()
    if self.LeftFashion and self.LeftFashion.itemlist then
        local itemCount = FGUI:GList_getNumItems(self.LeftFashion.itemlist)
        for i = 0, itemCount - 1 do
            local item = FGUI:GetChildAt(self.LeftFashion.itemlist, i)
            if item then
                local selectController = FGUI:getController(item,"select")
                if selectController then
                    local index = i + 1
                    if index == self.leftindex then
                        FGUI:Controller_setSelectedIndex(selectController, 1)  -- 选中状态
                    else
                        FGUI:Controller_setSelectedIndex(selectController, 0)  -- 正常状态
                    end
...
function FashionSystemPanl:ListCurAttrRenderer(idx,item)
    ...
    if fashion_charmlevel_data[charmData.charmLv]['attrlist'][idx+1] then
        ...
        local nameComp = FGUI:GetChild(item,"name")
        local valueTxt = FGUI:GetChild(item,"value")
        local nameTxt = FGUI:GetChild(nameComp, "title")
        ...
        FGUI:GTextField_setText(nameTxt, name)
        FGUI:GRichTextField_setText(valueTxt, "+"..value)
    end
end
...
function FashionSystemPanl:UpdateRightAttr()                      -- 更新右侧属性展示
    ...
    -- 更新激活属性列表
    FGUI:GList_itemRenderer(self.attr1_panl.attrfont, handler(self, self.ListActiveAttrRenderer))
    FGUI:GList_setDefaultItem(self.attr1_panl.attrfont, "ui://k09jn1qdrg4ij")
    FGUI:GList_setVirtual(self.attr1_panl.attrfont)
    FGUI:GList_setNumItems(self.attr1_panl.attrfont, #jhAttrTab)

    -- 穿戴属性 大于5条属性用 attr2_panl
    self.attr2_panl = FGUI:ui_delegate(self._ui.attr2_panl)       -- 穿戴属性组件
    ...
    -- 更新穿戴属性列表
    FGUI:GList_itemRenderer(self.attr2_panl.attrfont, handler(self, self.ListWearAttrRenderer))
    FGUI:GList_setDefaultItem(self.attr2_panl.attrfont, "ui://k09jn1qdrg4ij")
    FGUI:GList_setVirtual(self.attr2_panl.attrfont)
    FGUI:GList_setNumItems(self.attr2_panl.attrfont, #attData)
...
-- 激活属性列表渲染器
function FashionSystemPanl:ListActiveAttrRenderer(idx, item)
    local index = self.leftFashionList[self.leftindex][3]         -- 当前选中时装对应配置表下标
    local jhAttrTab = {}
    if self.rightPage == _FashionType.PiFeng then
        jhAttrTab = fashion_pifeng_data[index]['fashion_attr']
    elseif self.rightPage == _FashionType.HuanWan then
        jhAttrTab = fashion_huanwu_data[index]['fashion_attr']
    elseif self.rightPage == _FashionType.TouShi then
        jhAttrTab = fashion_toushi_data[index]['fashion_attr']
    end
    
    if jhAttrTab[idx+1] then
        local tab = jhAttrTab[idx+1]
        local nameTxt = FGUI:GetChild(item, "name")
        local valueTxt = FGUI:GetChild(item, "value")
        local name = attrConfigs[tab[1]]['Name'].."："
        local value = tab[2]
        local type = attrConfigs[tab[1]]['Type'] or 0 -- 0 数值 1 万分比
        ...
        FGUI:GTextField_setColor(nameTxt, "#666666")
        FGUI:GTextField_setText(nameTxt, name)
        FGUI:GTextField_setColor(valueTxt, "#0066cc")
        FGUI:GTextField_setText(valueTxt, "+"..value)
    end
end

-- 穿戴属性列表渲染器
function FashionSystemPanl:ListWearAttrRenderer(idx, item)
    local itemData = SL:GetValue("ITEM_DATA", self.leftFashionList[self.leftindex][1])
    local attData = FGUIFunction:GetAttShowData(itemData.Attribute, nil, nil)
    
    if attData[idx+1] then
        local nameTxt = FGUI:GetChild(item, "name")
        local valueTxt = FGUI:GetChild(item, "value")
        local name = attData[idx+1]['name'].."："
        local value = attData[idx+1]['value']
        FGUI:GTextField_setColor(nameTxt, "#666666")
        FGUI:GTextField_setText(nameTxt, name)
        FGUI:GTextField_setColor(valueTxt, "#0066cc")
        FGUI:GTextField_setText(valueTxt, "+"..value)
