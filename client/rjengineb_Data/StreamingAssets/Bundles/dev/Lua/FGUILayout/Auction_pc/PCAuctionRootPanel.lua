...
local PCAuctionRootPanel = class("PCAuctionRootPanel", BaseFGUILayout)
...
-- 拍卖行
function PCAuctionRootPanel:Create()
    self._ui = FGUI:ui_delegate(self.component)
    FGUIFunction:setWindowDrag(self.component, self._ui.bg)
    self._index = IDX_NULL
    self._pageDatas = {
        [IDX_BUY] = {
            objName = "PCAuctionBuyPanel",
            obj = nil,
        },
        [IDX_CONSIGNMENT] = {
            objName = "PCAuctionConsignMentPanel",
            obj = nil,
        },
        [IDX_RECORD] = {
            objName = "PCAuctionRecordPanel",
            obj = nil
        }
    }
...
function PCAuctionRootPanel:PageClose()
...
function PCAuctionRootPanel:PageTo(index)
...
function PCAuctionRootPanel:PageOpen(index)
    if self._index == index then return end
    self._index = index
    local pageData = self._pageDatas[self._index]
    ...
    local pageObj = pageData.obj
    if not pageObj then
        pageObj = FGUI:CreateObject(self.Node_Content,"Auction_pc",pageData.objName,true)
...
-- 获取所有的组件和控制器
function PCAuctionRootPanel:GetAllFGuiData()
...
function PCAuctionRootPanel:InitClickEvent()
...
function PCAuctionRootPanel:BtnRightTab2Clicked()
    self:PageTo(IDX_CONSIGNMENT)
end

function PCAuctionRootPanel:BtnRightTab3Clicked()
    self:PageTo(IDX_RECORD)
end

-- 切换页签
function PCAuctionRootPanel:SwtchTab(tabIndex)
...
function PCAuctionRootPanel:RemoveEvent()
end

function PCAuctionRootPanel:RegisterEvent()
end
...
function PCAuctionRootPanel:Exit()
...
function PCAuctionRootPanel:Destroy()
    self._ui = nil
...
function PCAuctionRootPanel:OnClose()
    self.super.Close(self)
end

return PCAuctionRootPanel
