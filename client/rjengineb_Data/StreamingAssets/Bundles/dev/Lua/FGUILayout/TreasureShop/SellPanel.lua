...
local SellPanel = class("SellPanel", BaseFGUILayout)
local ItemUtil = SL:RequireFile("FGUILayout/Item/ItemUtil")
local RES_COUNT = 24
function SellPanel:Create()
...
function SellPanel:GetAllFGuiData()
...
-- 切换页签显示
function SellPanel:RefreshTabChose(index)
...
function SellPanel:TabButtonClicked(index)
    self:RefreshTabChose(index)
    self:RefreshDataAndView()
end

function SellPanel:InitOnClickEvent()
...
function SellPanel:ComBoxFilterSelected()
...
-- 选中所有品质以下的物品
function SellPanel:FilterSellList()
    self._preSellList = {}
...
function SellPanel:CheckBoxClicked()
    if self.ctrl_checkbox.selectedIndex == 0 then
...
function SellPanel:RefreshCheckBox()
    if self.ctrl_checkbox.selectedIndex == 0 then
        self:FilterSellList()
    else
        self._preSellList = {}
...
function SellPanel:BtnTab1Clicked()
    self:TabButtonClicked(1)
end

function SellPanel:BtnTab2Clicked()
    self:TabButtonClicked(2)
end

function SellPanel:BtnTab3Clicked()
    self:TabButtonClicked(3)
end

function SellPanel:BtnTab4Clicked()
    self:TabButtonClicked(4)
end

function SellPanel:BtnTab5Clicked()
    self:TabButtonClicked(5)
end

function SellPanel:BtnTab6Clicked()
    self:TabButtonClicked(6)
end

-- 一键卖出
function SellPanel:BtnOnceSellClicked()
    self:SendToSell(nil)
end

function SellPanel:SendToSell(num)
...
function SellPanel:RefreshData()
    -- 预售列表
    self._preSellList = {}
    -- 能被卖的物品
...
-- 过滤数据
function SellPanel:TabFilterData()
...
function SellPanel:CleanItemCache()
...
function SellPanel:ResGetItemRender(idx,item)
    local index = idx + 1
...
function SellPanel:SellItemRender(idx, item)
    local index = idx + 1
    local node_root = FGUI:GetChild(item,"node_root")
...
function SellPanel:SellOnItem(data)
    local _data = {}
    _data.dialogType = 1
...
function SellPanel:SellItemClicked(item)
    local bg_selected = FGUI:GetChild(item,"bg_selected")
...
function SellPanel:SellSuccess()
    self:RefreshData()
    self:RefreshDataAndView()
end


function SellPanel:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_NPCSTORE_SELL_RES, "SellPanel", handler(self, self.SellSuccess))
end

function SellPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_NPCSTORE_SELL_RES,"SellPanel")
end

function SellPanel:Enter(serverGroupID)
