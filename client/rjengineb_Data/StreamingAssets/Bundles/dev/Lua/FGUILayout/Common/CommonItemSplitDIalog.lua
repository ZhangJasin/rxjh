...
-- 角色方案面板
function CommonItemSplitDialog:Create()
...
function CommonItemSplitDialog:InitData()
    self.num = 0
end

function CommonItemSplitDialog:GetAllFGuiData()
...
function CommonItemSplitDialog:CheckNum()
    if self.minNum then
...
function CommonItemSplitDialog:onBtnCloseClicked()
    self.super.Close(self)
end

function CommonItemSplitDialog:onBtnMinusClicked()
    self.num = self.num - 1
    if self.num <= 1 then
        self.num = 1
    end
...
function CommonItemSplitDialog:onBtnAddClicked()
    self.num = self.num + 1
    if self.num >= self.maxNum then
        self.num = self.maxNum
    end
...
function CommonItemSplitDialog:onBtnMaxClicked()
    self.num = self.maxNum
    self:RefreshCount()
end

function CommonItemSplitDialog:onBtnCancelClicked()
...
function CommonItemSplitDialog:onBtnFirstClicked()
...
function CommonItemSplitDialog:onBtnSecondClicked()
    if self._data.btnClicked then
        self._data.btnClicked(1, self.num)
    end
end

function CommonItemSplitDialog:onBtnSingleClicked()
    if self._data.btnClicked then
        self._data.btnClicked(1, self.num)
    end
end

function CommonItemSplitDialog:onClose()
    if self._data.btnClicked then
...
function CommonItemSplitDialog:Enter(_data)
    if _data then
        self._data = _data
    end
...
function CommonItemSplitDialog:CleanCache()
    if self.itemShow then
        ItemUtil:ItemShow_Release(self.itemShow)
    end
end

function CommonItemSplitDialog:RefreshItemNode()
    self:CleanCache()
    self.itemShow = ItemUtil:ItemShow_Create(self._data.itemData,self.iconNode,{OverLap = self._data.OverLap})
end

return CommonItemSplitDialog