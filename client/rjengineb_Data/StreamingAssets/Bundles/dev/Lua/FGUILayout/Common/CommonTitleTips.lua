...
local CommonTitleTips = class("CommonTitleTips", BaseFGUILayout)
local CELL_HEIGHT = 32


function CommonTitleTips:Create()
...
function CommonTitleTips:GetAllFGuiData()
...
function CommonTitleTips:InitData()
    self.attrData = {}
end
...
function CommonTitleTips:Enter(data)
    if data then
        self.data = data
    end
end

function CommonTitleTips:TitleAttributeItemRender(idx,item)
...
function CommonTitleTips:Refresh()
...
function CommonTitleTips:InitTouch()
    ...
    FGUI:setOnTouchEvent(self.mask, beginFunc, moveFunc, endFunc)
end

return CommonTitleTips