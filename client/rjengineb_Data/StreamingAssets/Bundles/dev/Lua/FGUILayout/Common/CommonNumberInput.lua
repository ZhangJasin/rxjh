...
local CommonNumberInput = class("CommonNumberInput", BaseFGUILayout)

local defaultMax = 999999
function CommonNumberInput:Create()
...
function CommonNumberInput:Enter(data)
	...
	self:SetShowNumber(data.curNum or 0)
	FGUI:GTextField_setText(self._ui.title, data.title)
end

function CommonNumberInput:Exit()
	
end

function CommonNumberInput:Close()
	self.super.Close(self)
end

-- 点击确认按钮
function CommonNumberInput:OnClickSureButton()
...
-- 点击取消按钮
function CommonNumberInput:OnClickCancelButton()
...
function CommonNumberInput:SetShowNumber(num)
	if num < 0 then
		num = 0
