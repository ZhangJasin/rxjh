...
local PCRechargePanel = class("PCRechargePanel", BaseFGUILayout)

PCRechargePanel.PAY_TYPE = {
    ALIPAY = 1,
    HUABEI = 2,
    WEIXIN = 3
}

function PCRechargePanel:Create()
...
function PCRechargePanel:InitProducts()
    -- 商品列表
...
function PCRechargePanel:OnInitListItem(idx, item)
    local index = idx + 1
...
function PCRechargePanel:OnClickListItem()
...
function PCRechargePanel:SelectProduct(selectIdx)
...
function PCRechargePanel:InitEvent()
    -- close
...
function PCRechargePanel:SelectChannel(channel)
...
function PCRechargePanel:OnInputMoneyEvent()
...
function PCRechargePanel:UpdateExchange()
    -- 选择货币
...
function PCRechargePanel:UpdateDesc()
    -- 选择货币
    if not self._productID then
...
function PCRechargePanel:OnClickSubmitPay()
