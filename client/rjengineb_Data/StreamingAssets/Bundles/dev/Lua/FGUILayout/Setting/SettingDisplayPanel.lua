...
local SettingDisplayPanel = class("SettingDisplayPanel", SettingPageBase)

function SettingDisplayPanel:Enter()
    SettingDisplayPanel.super.Enter(self)
    if not self.component then
        release_log_traceback("ERROR SettingDisplayPanel component is nil.")
        return
    end
...
function SettingDisplayPanel:Exit()
    SL:ComponentDetach(SLDefine.SUIComponentTable.SettingPickUp)

    SettingDisplayPanel.super.Exit(self)
end

function SettingDisplayPanel.Create()
    return SettingDisplayPanel.new()
end

function SettingDisplayPanel:InitData()
    self.infoTable = {{ -- 基础控制
        comName = "enBaseCtrl",
        key = "SETTING_BASE_EN",
        child = {{ -- 怪物名显示
...
function SettingDisplayPanel:InitEvent()
...
function SettingDisplayPanel:RefreshPanel()
...
function SettingDisplayPanel:InitItem(info, group, idx)
...
function SettingDisplayPanel:OnQuickWindowChanged(context)
    local enable = FGUI:GButton_getSelected(self._ui.tog_quickWindow)
    SL:SetValue("SETTING_QUICKWINDOW_NOT_REPEATED_SHOW", enable)
end

function SettingDisplayPanel:GetTogId(group, idx)
    return group * 1000 + idx
end

function SettingDisplayPanel:GetInfo(id)
    local idx = id % 1000
    local group = math.floor(id / 1000)
    return group, idx
end

function SettingDisplayPanel:OnClickEnemyCtrlHelpBtn(context)
    local data = {}
...
function SettingDisplayPanel:PlaySwitchTransition(widget, enable)
...
function SettingDisplayPanel:ResetSwitch(widget, enable)
    if FGUI:GButton_getSelected(widget) == enable then
        return
    end
    local transition = FGUI:GetTransition(widget, (enable == true) and "open" or "close")
