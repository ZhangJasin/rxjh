...
local SettingFightPanel = class("SettingFightPanel", SettingPageBase)
...
function ItemHandler:OnClickCheckBtn(context)
    ...
    FGUI:Open("Setting", "SettingSetItemOrderPanel", data)
end
...
function SettingFightPanel:Enter()
    SettingFightPanel.super.Enter(self)
    self._packageName = "Setting"
    if not self.component then
        release_log_traceback("ERROR SettingFightPanel component is nil. packageName:"..self._packageName)
        return
    end
...
function SettingFightPanel:Exit()
    SL:ComponentDetach(SLDefine.SUIComponentTable.SettingFight)

    SettingFightPanel.super.Exit(self)
end

function SettingFightPanel.Create()
    return SettingFightPanel.new()
end

function SettingFightPanel:Delegate()
...
function SettingFightPanel:InitData()
    self._handlers = {}
    self._helpSkillList = {}
...
function SettingFightPanel:InitEvent()
...
function SettingFightPanel:OnRecoverSkillLimitChanged(context)
...
function SettingFightPanel:OnRecoverSkillSelectChanged(context)
...
function SettingFightPanel:OnRecoverTeamSkillLimitChanged(context)
    local str = FGUI:GTextInput_getText(context.sender)
    local value = 0
    if str and string.len(str) then
        value = tonumber(str)
...
function SettingFightPanel:OnRecoverTeamSkillSelectChanged(context)
    local idx = FGUI:GComboBox_getSelectedIndex(context.sender)
	self._recover_team_skill_idx = idx
	self._recover_team_skill_select = self._recover_team_skill_id[idx+1]
...
function SettingFightPanel:RefreshPanel()
...
function SettingFightPanel:RefreshHelpSkill()
...
function SettingFightPanel:HelpListRender(idx, item)
...
function SettingFightPanel:OnHelpSkillEnableChanged(eventData)
...
function SettingFightPanel:SaveHelpSkillList()
    local list = {}
...
function SettingFightPanel:OnClickPickupCheckBtn(context)
    FGUI:Open("Setting", "SettingPickUpPanel")
end

function SettingFightPanel:OnAutoPickupEnableChanged(context)
    local enable = FGUI:GButton_getSelected(context.sender)
    SL:SetValue("SETTING_GLOBAL_AUTO_PICKUP_EN", enable)
end

-- 自动装箭开关
function SettingFightPanel:OnAutoAssembleArrowEnableChanged(context)
    local enable = FGUI:GButton_getSelected(context.sender)
    SL:SetValue("SETTING_AUTO_ASSEMBLE_ARROW_ENABLE", enable)
end

-- 查看箭头排序
function SettingFightPanel:OnClickAssembleArrowCheckBtn(context)
    local callBack = function(orderList)
...
function SettingFightPanel:ResetSwitch(widget, enable)
