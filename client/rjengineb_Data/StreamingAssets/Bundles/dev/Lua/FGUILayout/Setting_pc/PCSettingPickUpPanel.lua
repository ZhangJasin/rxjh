...
local PCSettingPickUpPanel = class("PCSettingPickUpPanel", BaseFGUILayout)

function PCSettingPickUpPanel:Create()
...
function PCSettingPickUpPanel:InitData()
...
function PCSettingPickUpPanel:InitEvent()
...
function PCSettingPickUpPanel:Enter(userdata)
	self:RegisterEvent()
	self:RefreshPanel()
end

function PCSettingPickUpPanel:Exit()

	self:RemoveEvent()
end

function PCSettingPickUpPanel:OnClose()
	self.super.Close(self)
end

function PCSettingPickUpPanel:Destroy()
end

function PCSettingPickUpPanel:RegisterEvent()
end

function PCSettingPickUpPanel:RemoveEvent()
end

function PCSettingPickUpPanel:RefreshPanel()
    FGUI:GList_setNumItems(self._list_content, self._list_eqlv_count + self._list_item_count + 1)
end

function PCSettingPickUpPanel:ContentProvider(idx)
    ...
    elseif idx == total then
        ...
        release_log("ERROR PCSettingPickUpPanel.ContentProvider Index out of range Idx:"..idx)
        return nil
    end
end

function PCSettingPickUpPanel:ItemRender(idx, item)
...
function PCSettingPickUpPanel:ItemRender_EqLv(idx, item)
    idx = idx + 1
...
function PCSettingPickUpPanel:OnEqLvShowDropItemSwitchChange(context)
...
function PCSettingPickUpPanel:OnEqLvPickSwitchChange(context)
    local btn = context.sender
    local idx = FGUI:GetIntData(btn)
    local id = self._list_eqlv[idx]
    local enable = FGUI:GButton_getSelected(btn)
...
function PCSettingPickUpPanel:OnClickCheckEqLvBtn(context)
    local btn = context.sender
    local idx = FGUI:GetIntData(btn)
    local id = self._list_eqlv[idx]
    
    FGUI:Open("Setting_pc", "PCSettingCheckPickUpPanel", id)
end

function PCSettingPickUpPanel:ItemRender_Item(idx, item)
...
function PCSettingPickUpPanel:OnItemShowDropItemSwitchChange(context)
    local btn = context.sender
    local idx = FGUI:GetIntData(btn)
    local id = self._list_item[idx]
    local enable = FGUI:GButton_getSelected(btn)
...
function PCSettingPickUpPanel:OnItemPickSwitchChange(context)
    local btn = context.sender
    local idx = FGUI:GetIntData(btn)
    local id = self._list_item[idx]
    local enable = FGUI:GButton_getSelected(btn)
...
function PCSettingPickUpPanel:ItemRender_Potion(idx, item)
...
function PCSettingPickUpPanel:HpListRender(idx, item)
    idx = idx + 1
...
function PCSettingPickUpPanel:OnHpItemSwitchChange(context)
...
function PCSettingPickUpPanel:MpListRender(idx, item)
    idx = idx + 1
...
function PCSettingPickUpPanel:OnMpItemSwitchChange(context)
    local idx = FGUI:GetIntData(context.data)
    local id = self._list_mp[idx]
    local enable = FGUI:GButton_getSelected(context.data)
...
function PCSettingPickUpPanel:OnNotSelectAllPickItemChange(context)
