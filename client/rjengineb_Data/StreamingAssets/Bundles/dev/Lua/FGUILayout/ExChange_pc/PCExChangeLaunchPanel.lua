...
function PCExChangeLaunchPanel:GetAllFGuiData()
    -- 上架数量
...
function PCExChangeLaunchPanel:InitData()
    self._mySells = {}
    self._scheduleSet = {}
end

-- 数据重构
function PCExChangeLaunchPanel:ResetData()
...
-- 初始化数据
function PCExChangeLaunchPanel:RefreshData()
    -- 目前上架条件共用拍卖条件
    self._selected_item_index = -1
    self._item_list = {}
end

function PCExChangeLaunchPanel:CleanCache()
...
function PCExChangeLaunchPanel:InitClickEvent()
...
function PCExChangeLaunchPanel:MyLaunchItemRender(idx, item)
...
function PCExChangeLaunchPanel:BagItemRender(idx, item)
    local index = idx + 1
    ...
    if data then
        ...
        FGUI:setOnRightClickEvent(self._item_list[id].component,function()
            FGUI:Open("ExChange_pc","PCExChangeLaunchDialog",data.BagData)
        end)
    else
...
function PCExChangeLaunchPanel:BagItemClicked(contextData)
...
function PCExChangeLaunchPanel:BtnRefreshClicked()
    self:RefreshBagView()
end

-- 刷新面板
function PCExChangeLaunchPanel:RefreshBagView()
...
function PCExChangeLaunchPanel:RefreshListMyLanuchView()
...
function PCExChangeLaunchPanel:Enter(data)
    self:RegisterEvent()
    self:RefreshData()
    SL:RequestExMySells()
end
...
--- 界面关闭时调用
function PCExChangeLaunchPanel:Exit()
    self:RemoveEvent()
    self:CleanCache()
end

function PCExChangeLaunchPanel:RegisterEvent()
    SL:RegisterLUAEvent(
        LUA_EVENT_EXCHANGE_MY_SELLS, "PCExChangeLaunchPanel", handler(self, self.RefreshListMyLanuchView)
    )
    
    SL:RegisterLUAEvent(
        LUA_EVENT_EXCHANGE_LAUNCH_REV, "PCExChangeLaunchPanel", handler(self, self.RefreshListMyLanuchView)
    )
end

function PCExChangeLaunchPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_MY_SELLS, "PCExChangeLaunchPanel")
    SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_LAUNCH_REV, "PCExChangeLaunchPanel")
end

--- 界面销毁时调用
function PCExChangeLaunchPanel:Destroy()
    self:CleanCache()
end

return PCExChangeLaunchPanel
