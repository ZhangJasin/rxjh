...
local PCExChangeRecordPanel = class("PCExChangeRecordPanel", BaseFGUILayout)
...
--- 界面被创建时调用
function PCExChangeRecordPanel:Create()
...
function PCExChangeRecordPanel:GetAllFGuiData()
...
function PCExChangeRecordPanel:InitData()
    self.list_item = {}
    self._filter = 0
end

function PCExChangeRecordPanel:InitClickEvent()
...
function PCExChangeRecordPanel:CleanCache()
...
function PCExChangeRecordPanel:RefreshRecordList()
...
function PCExChangeRecordPanel:RefreshRecordData(data)
    self._record  = data
    self:FilterDataAndRfView()
end

function PCExChangeRecordPanel:FilterDataAndRfView()
    self._filter_record = {}
...
function PCExChangeRecordPanel:SortData()
    table.sort(self._filter_record,function(a,b) return a.date > b.date end)
end

--- 界面打开时调用
function PCExChangeRecordPanel:Enter(data)
    self:RegisterEvent()
    SL:RequestExLog()
end

--- 界面关闭时调用
function PCExChangeRecordPanel:Exit()
    self:RemoveEvent()
    self:CleanCache()
end

function PCExChangeRecordPanel:RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_EXCHANGE_LOG, "PCExChangeRecordPanel", handler(self, self.RefreshRecordData))
end

function PCExChangeRecordPanel:RemoveEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_EXCHANGE_LOG, "PCExChangeRecordPanel")
end

--- 界面销毁时调用
function PCExChangeRecordPanel:Destroy()
end


return PCExChangeRecordPanel
