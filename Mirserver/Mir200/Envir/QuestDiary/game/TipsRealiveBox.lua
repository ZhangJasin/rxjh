...
function TipsRealiveBox.openshow(actor, data)
    ...
    -- 特权复活次数判断
    if index1 == 1 then
        local fhcount = gethumvar(actor, VarCfg.U_TeQuan_fuhuo_count) or 0  -- 特权可复活次数
        local yfhcount = gethumvar(actor, VarCfg.U_TeQuan_yifuhuo_count) or 0 -- 特权已复活次数
        if yfhcount >= fhcount then
            sendmsg(actor, 9, "特权复活次数不足")
            return
        end
        sethumvar(actor, VarCfg.U_TeQuan_yifuhuo_count, yfhcount + 1)
    end
...
GameEvent.add(EventCfg.onResetday, function(actor)
    sethumvar(actor, VarCfg.U_TeQuan_yifuhuo_count, 0)
end, TipsRealiveBox)
-- 死亡事件监听，打开复活提示框
