...
function mountMain.addsx(actor)
    ...
    for b=1,#classIds do
        setbuffabil(actor,110015,tonumber(classIds[b][1]),"=",tonumber(classIds[b][2]))
    end
    --坐骑幻化属性
            ...
            for r=1,#jhhhlist do
                ...
                for b=1,#classIds do
                    if hhsxListStr[classIds[b][1]] then
                        hhsxListStr[classIds[b][1]] = hhsxListStr[classIds[b][1]]+classIds[b][2]
                    else
...
function  getHHData(idx,grade)
    ...
    for i=1,#mountHHlist do
        if mountHHlist[i].Name == name and tonumber(mountHHlist[i].grade) == tonumber(grade) then
            data = mountHHlist[i]
            break
        end
    end
    ...
    if data then
        ...
        if getItemNum(actor,itemId)< num then
            ...
            if gethumvar(actor,VarCfg.U_Mount_IS_HH) == 1 then
                ...
                for i=1,#mountHHlist do
                    if mountHHlist[i].Name == name and mountHHlist[i].grade == grade - 1 then
                        oldHHModelId = mountHHlist[i].Model
                    end
                    if mountHHlist[i].Name == name and mountHHlist[i].grade == grade then
            ...
            Message.sendmsgEx(actor, "mountMain","updateHHmodel",{ycList=ycList,name = name,grade=grade,mountHHid = mountHHid})
        end
    else
...
function mountMain.setMountHHBuff(actor,oldbuffList,newBuffList,isCancel)
    if tonumber(isCancel) == 0 then
        for b=1,#oldbuffList do
            delbuff(actor,oldbuffList[b])
        end
        for c=1,#newBuffList do
            addbuff(actor,newBuffList[c])
        end
    else
        --取消幻化
        for b=1,#oldbuffList do
...
function mountMain.setModel(actor,data)
    ...
    if mountTakeId == data.mountId then
        ...
        for i =1,#mountHHlist do
            if mountHHlist[i].Model == data.mountId and allhhList[mountHHlist[i].Name] == mountHHlist[i].grade then
        ...
        if gethumvar(actor,VarCfg.U_Mount_IS_HH) == 1 then
            --原来已经有幻化了
            for i =1,#mountHHlist do
                if mountHHlist[i].Model == gethumvar(actor,VarCfg.U_Mount_Take_Id) and allhhList[mountHHlist[i].Name] == mountHHlist[i].grade then
                    if mountHHlist[i].buffID then
                        oldbuffList = mountHHlist[i].buffID
                    end
                end
    ...
    Message.sendmsgEx(actor, "mountMain","UpdateHHBtnName",{mountHHid = mountTakeId,isCancel=isCancel,oldModelId = oldMountTakeId })
end
...
function mountMain.jihuo(actor)
...
function mountMain.lsjihuo(actor,data)
    ...
    Message.sendmsgEx(actor, "mountMain","updateLSView",{ allPets = allPets, name = itemData.Pet_Name })
end
            ...
            if tonumber(v.Pet_Type) == 2 then
                for i = 1 ,#v.CoverPet_ActiveProType do
                    if result[v.CoverPet_ActiveProType[i]] then
                        result[v.CoverPet_ActiveProType[i]] = result[v.CoverPet_ActiveProType[i]] + v.CoverPet_ActiveProNum[i]
                    else
                        result[v.CoverPet_ActiveProType[i]] = v.CoverPet_ActiveProNum[i]
                    end
                end
            end
        end
            ...
            if tonumber(v.Pet_Type) == 1 then
                ...
                for i = 1 ,#v.BasePet_ProType do
                    if result[v.BasePet_ProType[i]] then
                        result[v.BasePet_ProType[i]] = result[v.BasePet_ProType[i]] + v.BasePet_ProNum[i] + v.BasePet_ProNum[i] * v.BasePet_GrowRatio[bl]
                    else
                        result[v.BasePet_ProType[i]] = v.BasePet_ProNum[i] + v.BasePet_ProNum[i] * v.BasePet_GrowRatio[bl]
                    end
                end
            --召唤的是幻化，要加上本体属性
            else
                ...
                for i = 1 ,#v.CoverPet_CoverProType do
                    if result[v.CoverPet_CoverProType[i]] then
                        result[v.CoverPet_CoverProType[i]] = result[v.CoverPet_CoverProType[i]] + v.CoverPet_CoverProNum[i]
                    else
                        result[v.CoverPet_CoverProType[i]] = v.CoverPet_CoverProNum[i]
                    end
                end
            end  
    ...
    if isAddBt then
        ...
        for i = 1 ,#btData.BasePet_ProType do
            if result[btData.BasePet_ProType[i]] then
                result[btData.BasePet_ProType[i]] = result[btData.BasePet_ProType[i]] + btData.BasePet_ProNum[i] * (btData.BasePet_GrowRatio[bl]+1)
            else
                result[btData.BasePet_ProType[i]] = btData.BasePet_ProNum[i] * (btData.BasePet_GrowRatio[bl]+1)
            end
        end
    end
    return result
...
function mountMain.updatePetModel(actor,data)
    ...
    Message.sendmsgEx(actor, "mountMain","updatePetModelResult",{allPetsHHData = allPetsHHData, showPetModelId = allPetsHHData[""..data.id]}) 
end
...
function mountMain.addPetToList(actor,monsterId,modelId)
    ...
    local newPet = { mark = mark, monsterId = monsterId, modelId = modelId, isDie = false, dieTime = 0}
        ...
        if hasPet[i].modelId == modelid then
            ...
            if hasPet[i].isDie then
                if isNow then
                    ...
                    if tonumber(dieTime) >= tonumber(SysConstant["PET_Resurre_CD"].Value) * 1000 then
                        realivepet(actor, mark)
                        hasPet[i].isDie = false
                        hasPet[i].dieTime = 0
                        sethumvar(actor, VarCfg.T_TAKE_PET, tbl2json(hasPet))
                        ...
                        Message.sendmsgEx(actor, methodName,"petResurrec", hasPet[i].dieTime) 
                        isShowDie = 2
                    end
                end
            else
            ...
            if data.isNeedBack and data.isNeedBack == 1 then
                Message.sendmsgEx(actor, "mountMain","recallpetResukt",{showPetModelId = modelid,selectViewPetId = data.btid}) 
            end
...
function mountMain.setPetAttr(actor,isShowDie)
    ...
    Message.sendmsgEx(actor, methodName,"setPetInfo",{type="red", max=max, now=now, icon = allAttr.icon}) 
end
...
function mountMain.levelUp(actor,data)
    ...
    if hasPet[data.name] == data.maxLv then
        ...
        Message.sendmsgEx(actor, "mountMain","level",{lv = hasPet[data.name], Pet_Name = data.name})
