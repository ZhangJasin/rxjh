...
function MentorShip.mapMoveAll(actor,data)
    ...
    for i=1,#data.users do
        ...
        sethumvar(UserID, VarCfg.S_FuBen_Var_PlayerPosition, tbl2json({targetinfo(actor, "NEWMAP"), targetinfo(actor, "X"), targetinfo(actor, "Y")})) -- 记录进入前位置信息
        mapmove(UserID, newMapId, 36, 37, 2) -- 传送进副本
        Message.sendmsgEx(UserID, "Invitation","moveResult")
        Message.sendmsgEx(UserID, "MainMission","EnterFuben",{isEnter=true,info={killnum = 0,killtype=data.taskType, StopTime = StopTime}})
    end
...
function MentorShip.addKillNum(actor,monidx)
    ...
    if mentorShipFuben ~= 0 then
        ...
        for i=1,#userList do
            ...
            Message.sendmsgEx(userList[i], "MainMission","ShowMentorShipInfo",{killnum = killnum,killtype = mentorShipFuben.taskType})
        end
    end
end
--count  ID
...
GameEvent.add(EventCfg.onLogin, function(actor)
   ...
   if mentorShipFuben ~= 0 then
        ...
        if mapid and checkmirrormap(mapid) then
            ...
            Message.sendmsgEx(actor, "MainMission","EnterFuben",{isEnter=true,info= {killnum = killnum,killtype=mentorShipFuben.taskType,StopTime = mentorShipFuben.StopTime}})
            -- dump(mapUserList)
